...
> ⚠️ **현행화 2026-09-22 — 이 영역은 애초에 변환 대상이 아니다.** Worker·IPC·broadcast·설정 모듈·Named Pipe 는 **Electron/Node 자원이라 그대로 유지**한다(00 문서 0절).
> 이 절은 지금도 **"무엇을 건드리면 안 되는지"를 판단하는 보존 기준**으로 쓴다. 변환 진척과 무관하게 계속 유효하다.
