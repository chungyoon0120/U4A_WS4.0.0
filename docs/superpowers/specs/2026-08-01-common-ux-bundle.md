...
### 6.0 이관 호환 규칙 (최우선)
- **★ 착수 전 구조 승인 게이트 (필수)**: 화면을 그리기 **전에** ① 팝업 **이름** 확정 →
  ② 대략적 **UX 구조**(영역 구성·배치·쓰는 공통 부품 목록)를 스케치 → ③ **소유자에게 전달해 확인**받는다.
  승인 후에만 실제 화면 구현. **근거**: 우리 프로젝트 구조와 동일하게 그리기 위해(임의 레이아웃 방지).
  구조가 애매하거나 대응 표준이 없으면 만들지 말고 되묻는다(§6.1-3).
