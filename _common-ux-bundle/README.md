...
기존 루트 `AGENTS.md` **맨 아래에 아래 한 줄**을 붙인다(루트에 `AGENTS.md` 가 없으면 이 내용으로 새로 만든다):

```
별창 팝업/화면 UX 요청 시 _common-ux-bundle/GUARDRAIL.md 를 먼저 읽고 그 규칙을 반드시 따를 것.
```

**왜 필수인가:** Codex 는 대화 시작 시 **루트 `AGENTS.md` 만** 자동으로 읽는다. 이 한 줄이 있어야
새 대화창에서 **"공통 UX 로 새 창 하나 만들어줘"** 라고만 해도 Codex 가 곧바로 규칙을 찾아 따른다.
(이 한 줄이 없으면 첫 마디에 "공통 UX" 가 무엇인지 못 알아들을 수 있다.)
