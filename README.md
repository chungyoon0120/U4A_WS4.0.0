...
# ⚠ 이 문서가 해당되는 경우 — 소스를 그대로 받아 실행할 때만

이 세팅 절차는 **설치 파일(exe)로 설치하지 않고, 소스를 받아서 그대로 실행하는 경우(개발용)** 에만 해당한다.

| 어떤 경우 | 이 문서 절차 |
|---|---|
| **소스를 받아 `npm start` 로 실행** (설치 안 함, 개발용) | **필요함** — 아래 전부 그대로 따라간다 |
| **설치 파일(exe)로 설치해서 사용** | **필요 없음** — 설치 파일 안에 이미 다 들어 있다. Python·C++ 빌드 도구·sqlite 다시 빌드 전부 안 해도 된다 |

즉 Python·C++ 빌드 도구를 깔고 sqlite 를 다시 빌드하는 건, **소스 그대로 돌리는 개발 PC를
차릴 때만** 하는 일이다. 만들어진 설치 파일을 받아 쓰는 사람에게는 시키지 않아도 된다.

---

# 새 PC에서 처음 세팅하기 (소스 그대로 실행용)
