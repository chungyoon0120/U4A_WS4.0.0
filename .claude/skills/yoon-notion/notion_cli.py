...
def resolve_token(name: str | None) -> tuple[str, str]:
    ...
    if not connections:
        raise NotionError(
            ...
            f"create one by writing {ENV_DIR}{os.sep}<name>.env containing a single line "
            f"'NOTION_TOKEN=ntn_...', then run:  {cli_hint()} check --as <name>"
        )
...
def cmd_connect_list(args) -> None:
    ...
    if not connections:
        ...
        print(f"추가:  {ENV_DIR}{os.sep}<이름>.env 파일을 만들고 아래 한 줄을 넣으세요")
        print("       NOTION_TOKEN=ntn_...")
        print(f"       그다음:  {cli_hint()} check --as <이름>")
        return
...
HELP_TEXT = """\
yoon-notion - 저장된 토큰으로 여러 노션 워크스페이스를 읽고 편집합니다.

사용법
  {cli} <명령> [인자] [--as <노션이름>] [--json]

  --as <노션이름>   어느 노션에 접속할지. 생략하면 기본 연결, 연결이 하나뿐이면 그걸 씀.
  --json            사람이 읽는 형식 대신 원본 JSON 출력.

연결 관리
  check             연결 테스트. 토큰 유효성 + 실제 페이지 접근 가능 여부까지 확인.
  connect list      등록된 노션 목록을 표로 출력.
  connect show <이름>   연결 하나의 상세 정보. 토큰 값은 표시하지 않음.
  connect use <이름>    이름 생략 시 쓸 기본 연결 지정.
  connect verify [이름] 토큰 유효성 확인 후 워크스페이스·봇 정보 갱신. --all 로 전체.
  connect remove <이름> 연결 삭제 (env 파일 삭제).
  whoami            현재 연결이 어떤 봇·워크스페이스에 붙는지 한 줄로.

읽기
  search [검색어]        페이지·DB 검색. --type page|database, --limit N
  get <URL또는ID>        페이지를 마크다운으로 읽기. --no-content 는 속성만.
  blocks <ID>            자식 블록을 ID와 함께 나열. 특정 블록을 고칠 때 먼저 사용.
  db <DB_ID>             데이터베이스 스키마(속성 목록) 확인.
  query <DB_ID>          데이터베이스 행 조회. --filter, --sorts 는 JSON.

쓰기
  append <ID>            페이지·블록에 마크다운 추가. --content-file 권장.
  create --parent <ID>   페이지 생성. --title, --content-file, --prop "이름=값"
  update <ID>            제목·속성 변경. --title, --prop, --archive, --restore
  replace <블록ID>       블록 한 개의 텍스트 교체. --text
  delete <블록ID>        블록 삭제.
  comment <페이지ID>     페이지에 댓글. --text

그 외
  raw <METHOD> <경로>    임의 Notion 엔드포인트 직접 호출. --body / --body-file
  help                   이 도움말.

예시
  {cli} check --as ws4.0
  {cli} connect list
  {cli} search "주간 회의록" --as ws4.0 --limit 10
  {cli} get https://www.notion.so/... --as ws4.0
  {cli} append <페이지ID> --as ws4.0 --content-file body.md
  {cli} create --parent <DB_ID> --title "새 항목" --prop "상태=진행중" --as ws4.0
  {cli} update <페이지ID> --as ws4.0 --title "바뀐 제목"
  {cli} query <DB_ID> --as ws4.0 --limit 25
  {cli} raw GET /v1/users --as ws4.0

노션 추가하기
  토큰은 명령줄에 입력하지 않습니다. 셸 기록과 대화 로그에 남기 때문입니다.
  1) {env}{sep}<노션이름>.env 파일을 만듭니다.
  2) 파일에 아래 한 줄을 넣습니다 (설명은 선택).
       NOTION_TOKEN=ntn_...
       DESCRIPTION=무슨 노션인지 한 줄
  3) {cli} check --as <노션이름> 으로 확인합니다.
  4) 노션에서 대상 페이지 ··· > Connections > 해당 통합을 추가해야 페이지가 보입니다.

마크다운 지원 (append / create 본문)
  제목 # ## ###, 문단, 글머리 - 목록, 번호 1. 목록 (2칸 들여쓰기로 중첩),
...
def cmd_help(args=None) -> None:
    print(HELP_TEXT.format(cli=cli_hint(), env=ENV_DIR, sep=os.sep))


def cmd_check(args) -> None:
    """End-to-end connection test: token valid AND pages actually reachable."""
    name, token = resolve_token(args.conn)
    report: dict = {"connection": name, "token_ok": False, "pages_visible": 0}
    lines = [f"연결 테스트: {name}", ""]

    me = api("GET", "/v1/users/me", token)  # raises with a clear message if bad
    bot = me.get("name") or "(이름 없음)"
    workspace = (me.get("bot") or {}).get("workspace_name") or "(알 수 없음)"
    report.update(token_ok=True, bot=bot, workspace=workspace, bot_id=me.get("id"))
    lines += [
        "  [OK]  토큰        유효",
        f"        워크스페이스  {workspace}",
        f"        통합(봇)      {bot}",
    ...
    pages = paginate("POST", "/v1/search", token, body={}, limit=5)
    report["pages_visible"] = len(pages)
    if pages:
        lines.append(f"  [OK]  페이지 접근  가능 ({len(pages)}건 이상 조회됨)")
        for item in pages[:3]:
            lines.append(f"          - [{item.get('object')}] {page_title(item)}")
    else:
        lines += [
            "  [!!]  페이지 접근  없음",
            "        토큰은 유효하지만 이 통합에 공유된 페이지가 하나도 없습니다.",
            "        노션에서 대상 페이지 ··· > Connections > 이 통합을 추가하세요.",
        ]

    # keep the stored metadata fresh, the way `connect verify` does
    path = env_path(name) if name != "$NOTION_TOKEN" else None
    if path and path.exists():
        values = parse_env_file(path)
        values.update(BOT=bot, WORKSPACE=workspace, VERIFIED_AT=now_iso())
        write_env_file(path, values)
        lines.append("")
        lines.append(f"        {path.name}의 워크스페이스·봇·확인일을 갱신했습니다.")

    emit(args, report, lines)
    if not pages:
        raise NotionError("연결은 됐지만 접근 가능한 페이지가 없습니다 (위 안내 참고)")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.set_defaults(func=cmd_help)   # bare invocation prints the help
    ...
    # --- notion
    p = sub.add_parser("help", help="명령어 목록과 사용 예시")
    p.set_defaults(func=cmd_help)

    p = sub.add_parser("check", parents=[common],
                       help="연결 테스트: 토큰 유효성 + 실제 페이지 접근 가능 여부")
