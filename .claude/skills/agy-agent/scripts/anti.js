...
async function runAgy(agyArgs, cwd, timeoutMs) {
    return new Promise((resolve) => {
        // agy `--print` takes the prompt as the VALUE of the flag (`--print "<text>"`),
        // NOT via stdin. If the prompt is piped to stdin and `--print` is left bare, agy
        // consumes the *next* flag as its prompt and answers about that flag instead.
        // So the prompt is passed inline (built into agyArgs right after "--print").
        //
        // We spawn with shell:false and agy's resolved full path, because:
        //   · A multi-line prompt (e.g. a diff) passed through cmd.exe (shell:true) gets
        //     truncated at the first newline — cmd treats it as a command separator.
        //     shell:false hands argv straight to CreateProcessW, so newlines survive.
        //   · shell:false won't do PATH/extension lookup on a bare "agy", so we resolve
        //     the first PATH match (the same copy a terminal would run) up front.
        //   · Bonus: no shell quoting/injection surface (fixes Node's shell+args warning).
        let agyCmd = "agy";
        try {
            const finder = process.platform === "win32" ? "where" : "which";
            const w = spawnSync(finder, ["agy"], { encoding: "utf8" });
            const first = (w.stdout || "").split(/\r?\n/).map((s) => s.trim()).filter(Boolean)[0];
            if (first) { agyCmd = first; }
        } catch (e) { /* fall back to bare "agy" */ }

        const child = spawn(agyCmd, agyArgs, {
            cwd,
            shell: false,
        ...
        // Prompt is passed inline via agyArgs (--print value), not stdin. Close stdin
        // so agy doesn't wait on it.
        child.stdin.end();
...
async function main() {
    ...
    // ★ prompt is the VALUE of --print (inline), not stdin — see runAgy for why.
    const agyArgs = ["--print", built.prompt, "--dangerously-skip-permissions"];
    if (!opts.write) {
    ...
    console.error(`[anti] running agy --print (prompt: ${built.prompt.length} chars, inline)`);

    let { code, timedOut, launchError, stdout, stderr } = await runAgy(agyArgs, cwd, opts.timeoutMs);
    ...
    if (!launchError && !timedOut && isAuthFailure(stderr)) {
        ...
        ({ code, timedOut, launchError, stdout, stderr } = await runAgy(agyArgs, cwd, opts.timeoutMs));
    }
