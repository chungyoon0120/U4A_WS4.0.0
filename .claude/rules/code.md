...
- **★★원본은 절대 손대지 않는다 — 예외 없음**(장군님 지시 2026-08-31, 무조건 최우선). 원본 = ① 원본 폴더 2곳 ② **작업 폴더 안 원본 사본(`www/ws30/ws10_20/design/`)과 원작 개발자 관리 팝업 폴더**. 원본은 계속 갱신되므로 **갱신본을 그대로 덮어 넣고 그 내용을 참고해 내 파일에서 개발**한다. 사본에 한 줄이라도 넣으면 갱신 때마다 백업·재분석 헛일이 반복된다. **급해서·잠깐만도 불가.** 사본에서 아직 불러 쓰는 곳이 있으면 그건 **변환 대상으로 올려 내 파일로 옮긴다**(사본 수정으로 때우지 말 것).
- **★원본 함수를 바꿔야 하면** 내 파일에서 **같은 이름에 새로 대입해 덮어쓴다**(원본 로드 완료 뒤 재설치). 원본이 나중에 덮어쓰는 경우엔 그 로드 콜백 안에서 다시 설치.
- **★원본 파일은 한 줄도 고치지 말고 새 파일로**(장군님 지시 2026-08-18). 원작 개발자가 `www/ws30/ws10_20/design` 등 원본 폴더를 **통째로 덮어써** 갱신하므로, 원본 안에 내 수정이 섞이면 갱신 자체가 불가능해진다. 내 파일은 원본 폴더 **밖**(`www/ws30/ws10_20/js/ws_html5_*.js` 계열)에 둔다. "원본이 경로로 파일을 읽어와서 어쩔 수 없다"는 이유도 **불가**: 읽어오는 창구는 `oAPP.fn.getScript`(`design/js/main.js` 288행) 하나뿐이고 실제 호출(1934·1937·1940행)은 나중 실행 함수 안이라, 내 파일에서 그 창구를 감싸 「원본경로→내 파일경로」 갈아끼우기 목록으로 대체할 수 있다. `oAPP.oDesign.pathInfo.designRootPath`(59행)를 통째로 바꾸면 원본 3,483개가 안 읽혀 죽으니 **파일별 목록만** 쓴다. 현황·잔여 목록 = `.works/원본덮어쓰기/00_현황판.md`.
