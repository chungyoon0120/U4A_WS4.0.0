...
argument-hint: <노션이름> <지시사항> | list | <노션이름> connect | help
allowed-tools: Bash(.claude/skills/yoon-notion/yoon-notion.sh:*), Bash(.agents/skills/yoon-notion/yoon-notion.sh:*), Bash(python:*), Bash(py:*), Read, Write, Glob
metadata:
  short-description: "저장된 토큰으로 여러 노션 워크스페이스를 이름으로 골라 읽고 편집한다."
---

# yoon-notion

이 스킬 폴더의 `env/<노션이름>.env`에 저장된 토큰으로 Notion API를 호출한다.
모든 API 호출은 아래 CLI를 통해서만 한다. 직접 `curl`이나 별도 스크립트를 쓰지 않는다.

```
<스킬폴더>/yoon-notion.sh <subcommand> --as <노션이름> [옵션]
```

`<스킬폴더>`는 이 SKILL.md가 있는 폴더다. 프로젝트 루트에서는 둘 중 존재하는 쪽을 쓴다
(같은 폴더를 가리킨다):

- `.claude/skills/yoon-notion/` — Claude Code
- `.agents/skills/yoon-notion/` — Codex
...
위 줄에 `$ARGUMENTS`라는 글자가 **치환되지 않고 그대로** 남아 있으면, 인자가 전달되지 않은
것이다(치환을 지원하지 않는 에이전트). 비어 있는 것으로 보고 대화 맥락에서 요청을 읽는다.

이 스킬은 워크스페이스마다 다른 토큰을 쓴다. **무엇을 하든 먼저** 등록된 이름을 확인한다:

```bash
<스킬폴더>/yoon-notion.sh connect list
```
...
- 인자가 있으면 **첫 단어**를 목록과 대조한다. 목록의 이름과 **정확히 일치**하면 그 이름이
  `--as` 값이고 나머지가 지시사항.
- 첫 단어가 `help`이거나, 관리 명령(`list`, `show`, `use`, `verify`, `remove`)이면
  아래 2번으로 간다. `<이름> connect` 형태면 그 이름으로 `check`를 돌린다.
...
| 사용자 입력 | 실행 |
|---|---|
| `help`, `도움말`, `명령어 뭐 있어` | `help` → 출력을 그대로 정리해 보여준다 |
| `list`, 목록 보기 | `connect list` → **3번 형식으로 답한다** |
| `<이름> connect`, 연결 테스트, 접속 확인 | `check --as <이름>` |
| 상세 보기 | `connect show <이름>` |
| 기본 연결 지정 | `connect use <이름>` |
| 삭제 | `connect remove <이름>` — **삭제 전 사용자에게 확인** |

### 토큰은 절대 대화나 명령줄로 받지 않는다

토큰을 추가하는 CLI 명령은 **일부러 없다.** 명령줄에 토큰을 쓰면 셸 기록과 대화 로그에
평문으로 남아 유출 사고가 되기 때문이다.

- 사용자가 채팅에 토큰을 붙여넣지 않았다면 **절대 요구하지 않는다.**
- 사용자가 채팅에 토큰을 붙여넣었다면, 그 값을 어떤 명령에도 넣지 말고
  **이미 노출됐으니 노션에서 폐기하고 새로 발급하라**고 알린다.
- 새 노션 등록은 아래 절차를 **안내만** 한다. 대신 실행해주지 않는다.

등록된 연결이 없거나 사용자가 추가를 원할 때 안내할 절차:
1. https://www.notion.so/profile/integrations 에서 New integration 생성
2. Internal Integration Secret(`ntn_`으로 시작) 복사
3. 편집기로 `<스킬폴더>/env/<노션이름>.env` 파일을 만들고 아래를 넣는다
   ```
   NOTION_TOKEN=ntn_...
   DESCRIPTION=무슨 노션인지 한 줄
   ```
4. `check --as <노션이름>` 으로 연결 확인
5. 노션에서 대상 페이지 `···` → **Connections** → 만든 통합 추가

토큰 값은 응답에 절대 출력하지 않는다. CLI도 출력하지 않는다(`connect show`는 `(저장됨)`만).
...
- **토큰은 목록에 넣지 않는다.** 마스킹된 형태라도 넣지 않는다. CLI도 출력하지 않는다.
  토큰이 실제로 유효한지는 `check`로 확인하는 것이지 눈으로 보는 게 아니다.
- 설명이 비어 있는 연결이 있으면 표 아래에 짚어주고, `env/<이름>.env`에
  `DESCRIPTION=...` 줄을 추가하면 된다고 알린다.
- 기본 연결(`*`)은 ✅로 표시한다.
- 값이 `-`인 칸은 그대로 `-`로 둔다.
- 표 아래에 한 줄로 요약한다: 총 몇 개, 기본 연결이 무엇인지.
- CLI가 "읽지 못한 파일"을 보고했으면 표 아래에 그 파일과 이유를 적고,
  `env/<이름>.env`를 편집기로 고치라고 안내한다.
- 워크스페이스·봇 칸이 비어 있으면 `check --as <이름>`으로 채울 수 있다고 알려준다.
...
```bash
<스킬폴더>/yoon-notion.sh search "주간 회의록" --as work --limit 10
```
...
```bash
<스킬폴더>/yoon-notion.sh get <페이지URL또는ID> --as work
<스킬폴더>/yoon-notion.sh blocks <페이지ID> --as work        # 블록 ID까지 필요할 때
<스킬폴더>/yoon-notion.sh db <데이터베이스ID> --as work       # DB 스키마
<스킬폴더>/yoon-notion.sh query <데이터베이스ID> --as work --limit 25
```
...
```bash
<스킬폴더>/yoon-notion.sh append <페이지ID> --as work --content-file <임시파일.md>
<스킬폴더>/yoon-notion.sh create --parent <부모ID> --title "제목" --as work --content-file <임시파일.md>
<스킬폴더>/yoon-notion.sh create --parent <DB_ID> --title "항목" --prop "상태=진행중" --prop "태그=a,b" --as work
```
...
```bash
<스킬폴더>/yoon-notion.sh update <페이지ID> --as work --title "새 제목" --prop "상태=완료"
<스킬폴더>/yoon-notion.sh replace <블록ID> --as work --text "바뀐 문장"
<스킬폴더>/yoon-notion.sh delete <블록ID> --as work
<스킬폴더>/yoon-notion.sh update <페이지ID> --as work --archive
```
...
```bash
<스킬폴더>/yoon-notion.sh raw GET /v1/users --as work
<스킬폴더>/yoon-notion.sh raw POST /v1/comments --as work --body-file <파일.json>
```
...
- `tests/test_notion_cli.py` — 네트워크 없이 도는 검증
  (런처가 아니라 파이썬으로 직접 실행: `python <스킬폴더>/tests/test_notion_cli.py`)
