...
rem   2. bundled runtime folder    self-contained; works with no Python installed
rem   3. local .venv folder        needs a base Python installation
...
rem   yoon-notion.cmd --setup-venv     create the optional .venv
rem
...
rem The bundled runtime. Unlike a venv this one is self-contained: it carries
rem its own standard library, so it runs on a machine with no Python at all.
...
echo. 1>&2
echo Looked at: YOON_NOTION_PYTHON, the bundled runtime folder, a local .venv, 1>&2
echo            py -3, python, python3, and uv. 1>&2
echo. 1>&2
...
echo   3^) Install uv       https://docs.astral.sh/uv/ 1>&2
echo. 1>&2
