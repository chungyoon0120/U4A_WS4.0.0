...
function _getPatchUpdateFileWorker(oPARAM){
    ...
    if(FS.existsSync(sExecutorNodeExePath) === false){
        ...
        document.dispatchEvent(new CustomEvent('update-error-SP', { 
            detail: { message: `[WORKER-002] ${GS_MSG.M23}` } 
        }));
...
function _getGlobalMsg(){
    ...
    GS_MSG.M23 = WSUTIL.getWsMsgClsTxt(sWsLangu, "ZMSG_WS_COMMON_001", "494") + "\n\n" + GS_MSG.M290; // 패치 업데이트 파일을 다운로드하는 데 필요한 실행 파일을 찾을 수 없습니다.   
...
exports.checkForUpdates = async function (remote, iscdn = false, versn, splev = 0, oLoginInfo) {
    ...
    /**
     * @since   2026-05-22 16:30:58
     * @version v3.6.3-0
     * @author  soccerhs
     * @description
     * 
     * 패치 진행 시, SQLite DB Connection 종료 시킨다.
     * 
     */
    WSUTIL.closeAllMsgClsDb();
