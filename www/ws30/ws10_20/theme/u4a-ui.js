...
(function (global) {
    ...
    function attachSuggest(oInput, fnItems, fnPick) {
        ...
        // 입력칸에 글자가 있으면 어떤 경로로 열리든(타이핑·포커스·아래화살표) 항상 그 글자로 좁힌다.
        //   [수정 2026-09-02] 예전엔 포커스로 열 때만 전체 이력을 다시 보여줘서, 글자를 치고
        //   목록을 좁혀 놓은 뒤 다른 곳을 눌렀다 입력칸으로 돌아오면 필터가 풀렸다
        //   (장군님 보고 2026-09-02 — 일반 검색창 관행에 맞춰 공통 전체 적용).
        function _filtered() {
            const aAll = fnItems() || [];
        ...
        oInput.addEventListener("input", () => { if (_suppressOpen) { return; } _open(); }); // 타이핑 → 부분일치 필터
        oInput.addEventListener("focus", () => _open());           // 포커스 → 값이 있으면 그 값으로 좁힌 목록, 비었으면 전체 이력
        oInput.addEventListener("keydown", (ev) => {
            switch (ev.key) {
                case "ArrowDown":
                    ev.preventDefault();
                    if (!oList) { _open(); } else { _setActive(Math.min(iActive + 1, aMatch.length - 1)); }
                    break;
                case "ArrowUp":
        ...
        function _renderVirtual(bKeepScroll) {
            ...
            if (!_vs || _vsWrap !== oWrap) {
                ...
                _vs = makeVScroller(oWrap, oUl, {
                    ...
                    // 재렌더 시 선택 강조를 <li> 껍데기가 아니라 안쪽 실제 행(.u4a-tree__row)에 붙인다(강조 CSS 대상과 일치).
                    //   이게 없으면 선택 후 패널 표시·리사이즈로 재렌더될 때 강조가 사라진다(직접 선택은 findRow 가 행에 붙여 정상).
                    selEl: function (oLi) { return oLi.querySelector(".u4a-tree__row") || oLi; },
                    makeSpacer: function () {
    ...
    function createField(opts) {
        ...
        if (sType === "combo" || sType === "select") {
            // 밸류스테이트(오류표시)를 생성 시점부터 콤보에 전달 — 초기 오류상태도 그 칸에 표시.
            const oCombo = createSelect(opts.items || [], opts.value, opts.onChange,
                { onOpen: opts.onOpen, valueState: opts.valueState, valueStateText: opts.valueStateText });
            ...
            return {
                ...
                setReadOnly() { },
                // 오류표시는 공통 콤보(createSelect)가 이미 지원 — 빈 함수로 덮지 말고 그대로 위임.
                //   (드롭다운 칸 자체에 빨간 테두리 + 펼침목록 최상단 오류문구. 원본 UI5 ComboBox valueState 대응.)
                setValueState(sState, sText) { if (oCombo.setValueState) { oCombo.setValueState(sState, sText); } },
                focus() { oCombo.focus(); }
            };
        }
    ...
    function openColumnMenu(oCol, oTh, ctl, opts) {
        ...
        // [옵션] 필터 표시자(깔때기) 자동 관리 — opts.filterIndicator 일 때만(기본 꺼짐=기존 소비처 무영향).
        //   필터 걸린 컬럼 헤더에만 공통 .u4a-th__ind(fa-filter · --accent)를 붙인다(상시 아이콘 금지).
        //   화면마다 손으로 붙이던 것을 공통화 — 앞으로 컬럼메뉴만 쓰면 깔때기가 자동으로 뜬다(장군님 지시 2026-08-13).
        function _syncFilterInd() {
            if (!opts.filterIndicator || !oTh || !bFilter || !ctl.getFilter) { return; }
            const bOn = !!ctl.getFilter(oCol.key);
            let oInd = oTh.querySelector(".u4a-th__ind");
            if (bOn) {
                if (!oInd) { oInd = _el("span", "u4a-th__ind"); oInd.innerHTML = _fa("filter"); oTh.appendChild(oInd); }
        ...
        if (bFilter) {
            ...
            // ★필터는 Enter 로만 적용(장군님 지시 2026-08-13). 포커스 빠짐(blur=Tab/딴곳 클릭)으로는 적용하지 않는다.
            fw.appendChild(fi);
            m.appendChild(fw);
        }
        ...
        if (bFilter) {
            ...
            clr.addEventListener("click", function () {
                ...
                ctl.setFilter(oCol.key, ""); fi.value = ""; ctl.rerender(); _syncFilterInd(); closeColumnMenu();
            });
            m.appendChild(clr);
        }
        ...
        m.style.zIndex = "10";
        _oColMenuEl = m;
        _syncFilterInd();   // 현재 필터 상태를 헤더 깔때기에 반영(옵션 시).
