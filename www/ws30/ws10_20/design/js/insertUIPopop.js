(function(){
  ...
  oAPP.fn.callUIInsertPopup = function(UIOBK, retFunc){
     ...
     */
      ...
      //  * 해당 UI만 추가 가능토록 로직 보완함.
      //  */
      // let _aUW10 = [];

      // if(oAPP.common.checkWLOList("C", "UHAK901289") === true){
      //   _aUW10 = oAPP.attr.S_CODE.UW10.filter( item => item.FLD01 === UIOBK && item.FLD03 === ls_sel.UIATT && item.FLD06 !== "X" );
      // }
      ...
      let _checkAllowChildAggr = parent.require(_modulePath).checkAllowChildAggr;
      ...
      for(var i = 0, l = lt_0027t.length; i < l; i++){
        ...
        //대상 UI의 Aggregation에 허용 가능 UI 항목(UW10)의 공통 점검 로직 처리를 위해 기존로직 주석 처리.
        // //특정 UI만 추가 가능 예외에 해당하는경우 해당 UI가 아닌경우 항목 수집 skip.
        // if(_aUW10.length > 0 && _aUW10.findIndex( item => item.FLD04 === ls_0022.UIOBK ) === -1){
        //   continue;
        // }
        ...
        //추가 대상 AGGREGATION.
        _sParam.UIATT = ls_sel.UIATT;

        //추가 UI.
        _sParam.UIOBK = ls_0022.UIOBK;

        //부모의 Aggregation에 추가 불가능한 UI인지 확인.
        if(!_checkAllowChildAggr(_sParam)){
          continue;
        }
    ...
    var oInp2 = new sap.m.Input({
      showClearIcon : true, 
      /**
       * @since   2026-04-20 03:36:18
       * @version v3.6.0-4
       * @author  PES
       * @description
       * UI명 검색시 suggestion 기능에 의해 값을 입력하는데 불편한 사항이 있기에
       * autocomplete을 false로 설정.
       */
      autocomplete : false
    });
     ...
     * @author  PES
     * @description
     * ui attribute 개인화 적용 여부 체크박스 추가.
     */
    var _txt = parent.WSUTIL.getWsMsgClsTxt("", "ZMSG_WS_COMMON_001", "812");

    var _oCheckbox = new sap.m.CheckBox({
      //812  개인화 속성 적용
      text : _txt,
      tooltip : _txt,
      selected:lf_getAttrPreset() //UI Attribute 개인화 항목 적용 여부 체크 함수, true/false 반환
    });

    oFrmElem3.addField(_oCheckbox);
    ...
    oLTDrag1.attachDragStart(function(oEvent){
      ...
      lf_rowDragStart(oEvent, oDlg, oInp1, _oCheckbox);
    ...
    function lf_selectUi(is_UI){
      ...
      //UI Attribute 개인화 항목 localStorage 저장 처리.
      lf_saveAttrPreset(_oCheckbox);
      ...
      //return parameter 전달.
      retFunc(ls_0022, ls_0023, l_cnt, lt_0023, _oCheckbox.getSelected());
  ...
  //결과리스트 table 라인 drag start 처리.
  function lf_rowDragStart(oEvent, oDlg, oInp1, oCheckbox){
    ...
    event.dataTransfer.setData("text/plain", "callUIInsertPopup|" + ls_0022.UIOBK + "|" + l_cnt + "|" + oAPP.attr.DnDRandKey + "|" + oCheckbox.getSelected());

    
    //UI Attribute 개인화 항목 localStorage 저장 처리.
    lf_saveAttrPreset(oCheckbox);
  ...
  //Apply Personalized Attributes 체크박스 선택값 localStorage 저장 처리.
  function lf_saveAttrPreset(oCheckbox){

    localStorage.setItem("U4A_APPLY_P13N_ATTR", JSON.stringify({
      VALUE : oCheckbox.getSelected()
    }));

  }


  //Apply Personalized Attributes 체크박스 선택값 localStorage에서 얻기 처리.
  function lf_getAttrPreset(){

    var _sValue = localStorage.getItem("U4A_APPLY_P13N_ATTR");
    ...
    if(typeof _sValue.VALUE === "undefined"){
      return false;
    }

    return _sValue.VALUE;
