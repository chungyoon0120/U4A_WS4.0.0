(function(){
    ...
    oAPP.fn.callSetSameAttrPopup = function(is_attr){
        ...
        var oDlg = new sap.m.Dialog({resizable:true, draggable:true, contentWidth:"60%",
            contentHeight:"60%",verticalScrolling:false}).addStyleClass("sapUiSizeCompact");
        ...
        async function lf_setBindVal(){
            ...
            //해당 라인이 이벤트에 해당하는 라인인경우.
            if(is_attr.UIATY === "2"){

                _ISLST = "X";

                //서버이벤트 항목 검색.
                ls_binfo.T_DDLB = await new Promise((res)=>{
        ...
        async function lf_setSyncAttr(){
            ...
            for(var i = 0, l = lt_sel.length; i < l; i++){
                ...
                let _pos = oAPP.attr.prev[ls_line.OBJID]._T_0015.findIndex( a => a.UIATK === ls_line.UIATK && a.UIATY === ls_line.UIATY );
                
                var ls_0015 = oAPP.attr.prev[ls_line.OBJID]._T_0015[_pos];
            ...
            //이벤트에 해당하는건인경우.
            if(is_attr.UIATY === "2"){
                //현재 구성한 서버이벤트 ddlb를 같이 구성 처리.
                is_attr.T_DDLB = JSON.parse(JSON.stringify(oMdl.oData.T_DDLB));
            }
