(function(){
  ...
  oAPP.fn.uiPreviewArea = function(oMPage){
    ...
    /**
     * @since   2026-01-12 09:59:39
     * @version v3.5.7-4
     * @author  pes
     * @description
     * 단축키 잠금 상태를 표현 하기 위한 아이콘 추가.
     * (단축키가 잠금 처리 된경우를 시각적으로 확인하기 위함)
     */
    let _oAvatar = new sap.m.Avatar({
      customDisplaySize:"2rem",
      displaySize:"Custom",
      src : "sap-icon://locked",
      visible : false
    }).addStyleClass("sapUiTinyMarginBegin");
    oTool.addContent(_oAvatar);
  ...
  // oAPP.fn.loadPreviewFrame = function(){
    ...
    if(oFrame?.contentWindow?._loaded === true){
      //라이브러리 로드 처리.
      ...
      oFrame.contentWindow.drawPreview().then(()=>{
        ...
        oAPP.attr.ui.oLTree1.fireCellClick({rowBindingContext:_oCtxt});
  ...
  oAPP.fn.prevStyleClassApply = function(it_css, bSave){
    ...
    for(var b=0, bl=lt_OBJID.length; b<bl; b++){
      ...
      //styleClass 프로퍼티가 바인딩된 건 수집.
      if(ls_0015_b && ls_0015_b.ISBND === "X"){ lt_bound.push(lt_OBJID[b].OBJID); }
  ...
  oAPP.oDesign.fn.prevRemoveUiObject = function(is_tree){
    ...
    return new Promise(async (resolve)=>{
        ...
        /**
         * @since   2026-02-03 00:56:34
         * @version v3.5.9-1
         * @author  PES
         * @description
         * 미리보기의 UI를 삭제 처리한뒤 UI가 지워지는것을 기다리는 로직 처리 후 
         * UI를 다시 추가 하는 과정에서 richtextEditor UI 존재시 richtextEditor의 예외처리
         * 로직이 수행되지 않는 내용에 의해 기존 UI가 삭제되는것을 기다리는 로직 주석 처리.
         */
