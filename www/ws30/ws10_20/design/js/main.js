(function(oAPP){
  ...
  oAPP.fn.main = function(){
    ...
    oAPP.fn.crtStru0010 = function(){
      /**
       * @since   2026-02-06 20:14:28
       * @version v3.6.0-0
       * @author  pes
       * @description
       * 서버이벤트 수행중 덤프 발생시 ST22에 덤프 집계 처리 여부 필드(/U4A/T0010-DMPWT) 추가.
       */
      ...
      var xhttp = new XMLHttpRequest();
        xhttp.onreadystatechange = function() {
          if (this.readyState == 4 && this.status == 200) {
            ...
            /**
             * @since   2026-03-24 12:01:46
             * @version v3.6.0-4
             * @author  pes
             * @description
             * js파일 로드시 sourceURL을 적용하여, 개발자 도구에서 로드된 js파일의 이름이 보이도록 처리하는 로직 추가.
             * vs code에서 Ctrl+마우스 우클릭으로 해당 source로 navigation 하는 경우
             * source 하단에 //# sourceURL=로 파일의 절대경로가 명시되어 있어야 navigation이 
             * 가능하기 때문에, js파일을 로드할 때 해당 js파일의 절대경로를 sourceURL로 적용하는 로직 추가.
             */
    ...
    oAPP.fn.getAppData = function(){
      ...
      sendAjax(oAPP.attr.servNm + "/getAppData", oFormData, async function(param){
        ...
        //테스트!!!!!!!!!!
        console.timeEnd("APP DATA 로드 시간");

        //테스트!!!!!!!!!!
        console.time("APP DATA 처리 시간");
        ...
        //테스트!!!!!!!!!!
        console.timeEnd("APP DATA 처리 시간");
        ...
        /**
         * @since   2026-03-25 11:48:48
         * @version v3.6.0-4
         * @author  pes
         * @description
         * 어플리케이션 DB 정보 검색을 위해 서버 호출시 GET 방식으로 호출하도록 변경.
         * 기존 POST 방식에서 GET 방식으로 변경하여 서버 호출하도록 수정.
         * 추후 어플리케이션 변경 여부를 판단하여 서버 캐시 적용이 가능하도록 하기 위함.
         */
    ...
    oAPP.fn.getLibData = function(it_lib, is_tab, dbtot, dbcnt, fkey, skey){
      ...
      sendAjax(oAPP.attr.servNm + "/getLibData", oFormData, function(param){
         ...
         * @author  pes
         * @description
         * 라이브러리 DB 정보 검색을 위해 서버 호출시 GET 방식으로 호출하도록 변경.
         * 기존 POST 방식에서 GET 방식으로 변경하여 서버 호출하도록 수정.
         * 패치등의 사유로 라이브러리 정보가 변경 되는 경우에만 서버에서 직접 라이브러리 정보를 검색하도록 하기 위함.
         * (라이브러리 DB 정보가 변경되지 않은 경우에는 캐시된 정보를 사용)
         */
      }, "X", true, "GET", function(e){
    ...
    oAPP.fn.setUIAreaEditable = function(isRefresh){
      ...
      //테스트!!!!!!!!!!!!!!!
      console.time("미리보기 로드 시간");
      console.time("라이브러리 로드 시간");
      //테스트!!!!!!!!!!!!!!!
      ...
      if(oAPP.attr.appInfo.IS_EDIT === "" &&
         ...
         isRefresh !== "X" ){
          ...
          //attribute 비활성 처리.
          for (const _sAttr of oAPP.attr.oModel.oData.T_ATTR) {
              _sAttr.edit = false;
          }
    ...
    oAPP.fn.removeContent = function(){
      ...
      /**
       * @since   2026-03-25 11:40:08
       * @version v3.6.0-4
       * @author  pes
       * @description
       * WS_20 -> WS_10 화면 이동시 라이브러리 정보 제거 로직 추가.
       * WS_10 -> WS_20 으로 화면 이동시 라이브러리 정보를 얻는 방식이
       * 서버 캐시를 활용하여 라이브러리 정보를 얻는 방식으로 변경됨에 따라,
       * 더이상 메모리에 라이브러리 정보를 보관할 필요가 없어짐.
       */
      if(oAPP.common.checkWLOList("C", "UHAK901369")){
        delete oAPP.DATA.LIB;
      }
      ...
      if(typeof oAPP?.attr?.ui?.frame?.src === "undefined"){
        ...
        /**
         * @since   2026-02-10 12:03:00
         * @version v3.6.0-0
         * @author  pes
         * @description
         * WS_20 -> WS_10 화면 이동 전 미리보기 HTML의 IFRAME의 SRC 정보가 존재하지 않는
         * 치명적 오류 메시지 처리 추가.
         */
        let _msg = `"www/ws10_20/design/js/main.js"\n` +
          `파일에서 오류 발생.\n` +
          `WS_20 -> WS_10 화면 이동 전 미리보기 HTML의 IFRAME의 SRC 정보가 존재하지 않음\n` +
    ...
    oAPP.fn.getAttrChangedData = function(){
      ...
      for(var i in oAPP.attr.prev){
        ...
        for(var j=0, l2= oAPP.attr.prev[i]._T_0015.length; j<l2; j++){
           ...
           * object 정보를 JSON string화 하는 로직 추가.
           */
