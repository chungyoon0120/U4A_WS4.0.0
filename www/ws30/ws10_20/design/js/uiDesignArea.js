(function(){
  ...
  oAPP.fn.uiDesignArea = function(oLPage){
    ...
    oLTDrag1.attachDragStart(function(oEvent){
      ...
      //text/plain으로 미리보기 영역과 design tree 영역으로 D&D 정보를 전달받고 있음.
    ...
    // /**
    //  * @since   2026-04-21 16:14:08
    //  * @version v3.6.0-4
    //  * @author  pes
    //  * @description
    //  * 프로퍼티 개인화 기능을 위한 버튼 추가.
    //  * (테스트 단계)
    //  */
    // if(parent.REMOTE.app.isPackaged === false){
    //   //ATTR 개인화 테스트 버튼.
    ...
    //       //프로퍼티 개인화 팝업 호출 처리.
    //       parent.require(parent.PATH.join(parent.getPath("POPUP_ROOT"), 
  ...
  oAPP.fn.drop_cb = async function(param, i_drag, i_drop){
     ...
     */
    let _allow = parent.require(_modulePath).checkAllowChildAggr({PUIOK : i_drop.UIOBK, UIATT:param.UIATT, UIOBK: i_drag.UIOBK});
  ...
  oAPP.fn.designAddUIObject = async function(is_tree, is_0022, is_0023, i_cnt){
    ...
    // ui attribute 개인화 적용 여부 확인.
    let _isPresetAttr = arguments[4] || false;
     ...
     * 입력한 UI의 대상 aggregation인경우 허용 가능한지 여부를 판단하는 로직 추가.
     */
    let _allow = parent.require(_modulePath).checkAllowChildAggr({PUIOK : is_tree.UIOBK, UIATT:is_0023.UIATT, UIOBK: is_0022.UIOBK});
    ...
    for(var i = 0; i < l_cnt; i++){
      ...
      if(_isPresetAttr){
        ...
        let _filePath = parent.PATH.join(parent.PATHINFO.P13N_ROOT, "UI_ATTR", "UI_ATTR_PRESET.db");

        //프로퍼티 개인화 DB가 존재하는 경우
        if (parent.FS.existsSync(_filePath)) {

          let _sqlite = new sqliteManager(_filePath);
          
          let _aPreset = [];
          
          _aPreset = _sqlite.selectData({
          ...
          for(let _sPreset of _aPreset){

            let _UIATK = _sPreset.UIATK;
      ...
      //UI 생성 처리.
      oAPP.attr.ui.frame.contentWindow.createUIInstance(l_14, _aT0015);
  ...
  oAPP.fn.designUIDropInsertPopup = function(oEvent, OBJID){
    ...
    function lf_setChild(is_0023){
      ...
      oAPP.fn.designAddUIObject(ls_drop, ls_0022, is_0023, l_cnt, _isPresetAttr);
    ...
    //Drag한 정보 발췌.
    var lt_dragInfo = oAPP.fn.getDragParam(oEvent);
    if(!lt_dragInfo || lt_dragInfo.length !== 5){
      //WS 20 -> 바인딩 팝업 BUSY OFF 요청 처리.
      parent.require(oAPP.oDesign.pathInfo.bindPopupBroadCast)("BUSY_OFF");
      return;
    }
    ...
    // ui attribute 개인화 적용 여부 확인.
    let _isPresetAttr = lt_dragInfo[4] === "true" ? true : false;
  ...
  oAPP.fn.designAddTreeData = function(is_data, is_tree, sAggr){
    ...
    function lf_aggrPopup_cb(param, i_cdata){
         ...
         * 입력한 UI의 대상 aggregation인경우 허용 가능한지 여부를 판단하는 로직 추가.
         */
        let _allow = parent.require(_modulePath).checkAllowChildAggr({PUIOK : is_tree.UIOBK, UIATT:param.UIATT, UIOBK: i_cdata.UIOBK});
  ...
  oAPP.fn.designUIAdd = function(is_tree){
    ...
    function lf_setChild(is_0022, is_0023, i_cnt){
      ...
      //UI 추가 처리 FUNCTION 호출.
      oAPP.fn.designAddUIObject(is_tree, is_0022, is_0023, i_cnt, arguments[4]);
