...
// 브라우저 키
let sBrowserKey = CURRWIN?.webContents?.getWebPreferences?.()?.browserkey || "";
if(!sBrowserKey){
    sBrowserKey = oQueryParams.browserkey;
}
...
function createChannel() {
    ...
    oChannel.onmessage = function(oEvent) {
        ...
        switch (oEvent.data.PRCCD) {
            ...
            case "U4A_HELP_DOC_OPEN":
                //U4A HELP DOCUMENT 호출 처리.
                responeCallU4AHelpDoc(oEvent);                
...
/************************************************************************
 * @function - U4A HELP DOCUMENT 호출 처리.
 ************************************************************************/
function responeCallU4AHelpDoc(oEvent){

    /**
     * @since   2026-04-29 16:42:20
     * @version v3.6.0-4
     * @author  pes
     * @description
     * 도움말 HTML 정보를 U4A HELP DOCUMENT로 통합하게 됨에 따라
     * 바인딩 팝업 -> DESIGN 영역으로 도움말 팝업 호출에 요청에 대한
     * 팝업 호출 처리 로직 추가.
     */

    let sP13n_root = parent.getPath("P13N_ROOT");
    let sUrlPath = parent.getPath("U4AHELP");
    let oHelpDoc = parent.require(sUrlPath);

    let _isBusy = parent.getBusy();

    //현재 DESIGN 영역에 BUSY가 호출되지 않은경우 BUSY ON 처리.
    //(바인딩 팝업에서 동일속성 적용 화면으로 이동하게 되는 경우 DESIGN 영역을
    //동일속성 적용 화면을 벗어나기 전까지 BUSY처리를 유지하는 내용이 있기에
    //BUSY처리가 유지되고 있다면 별도의 BUSY를 제어하지 않고, BUSY가 켜있지
    //않은 상황이라면 BUSY를 제어하는 로직 처리함.)
    if(_isBusy === ""){
        parent.setBusy("X");
    }


    oHelpDoc.Excute(REMOTE, sP13n_root, oEvent?.data?.sParam?.opstion).then(function(res){

        //DESIGN 영역 -> 바인딩팝업 BUSY OFF 요청 처리.
        sendBindPopupBusyOff();
...
module.exports = function(PRCCD, oData){
    ...
    switch (PRCCD) {
        ...
        case "U4A_HELP_DOC_OPEN":
            /**
             * @since   2026-04-29 16:44:58
             * @version v3.6.0-4
             * @author  pes
             * @description
             * 도움말 HTML 정보를 U4A HELP DOCUMENT로 통합하게 됨에 따라
             * 바인딩 팝업 -> DESIGN 영역으로 도움말 팝업 호출 요청 로직 추가.
             */
            sendPostMessage({PRCCD:PRCCD, sParam:oData});
            break;
