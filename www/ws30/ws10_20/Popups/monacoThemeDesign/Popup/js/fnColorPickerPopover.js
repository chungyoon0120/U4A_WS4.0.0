/************************************************************************
 * monacoThemeDesign/Popup/js/fnColorPickerPopover.js  (HTML5)
 * ----------------------------------------------------------------------
 *  공통 컬러 피커 팝오버 — **WS20 속성 값도움에 쓰는 js/fnColorPickerPopover.js 를 그대로 이식**
 *  (장군님 지시: 기존 공통 색상선택기 재사용). 원본 sap.ui.unified.ColorPickerPopover 대체.
 *
 *  · 별창(다른 document/realm)이라 WS20 renderer 의 oAPP.fn.fnColorPickerOpen 을 직접 못 쓴다
 *    → 코드를 복사한다. 색 연산·DOM 구조·클래스(.u4aCp*)·CSS·동작은 원본과 100% 동일 →
 *    WS20 속성 값도움과 시각·조작이 완전히 같다.
 *  · 적응한 부분(그 외 무변경):
 *      - 진입점 oAPP.fn.fnColorPickerOpen → window.U4AColorPicker.open (별창엔 oAPP 없음)
 *      - _wsTxt 의 parent.getUserInfo()/parent.WSUTIL → 별창 컨텍스트(require WSUTIL + 쿼리 USERINFO)
 *      - 미사용 인자 $(jQuery) 제거(별창은 jQuery 미로드)
 *
 *  진입: U4AColorPicker.open(oAnchorEl, sInitColor, fnConfirm)
 *    @param {HTMLElement} oAnchorEl   - 위치 기준 DOM. 없으면 화면 중앙.
 *    @param {string}      sInitColor  - 초기 색(#rrggbb / ""). 빈값이면 #ffffff.
 *    @param {function}    fnConfirm   - OK 시 fnConfirm("#rrggbb") 호출(원본 hex param 동일).
 *
...
(function (window) {
  ...
  // ── 별창 컨텍스트(원본의 parent.WSUTIL / parent.getUserInfo 대체) ─────
  ...
  var _oQuery = WSUTIL.QueryString.parse(window.location.href);
  var _LANGU = (_oQuery.USERINFO && _oQuery.USERINFO.LANGU) || "";
  ...
  function _wsTxt(sCode) {
    try {
      return WSUTIL.getWsMsgClsTxt(_LANGU, "ZMSG_WS_COMMON_001", sCode, "") || "";
    } catch (e) { return ""; }
  }
   ...
   * 공개 진입점 — 컬러 피커 팝오버 열기.
   ************************************************************************/
  function fnColorPickerOpen(oAnchorEl, sInitColor, fnConfirm) {
  ...
  window.U4AColorPicker = { open: fnColorPickerOpen };
