            ...
            function lf_onResult(oEvent, oRES){
                ...
                if(oRES.RETCD !== "S"){
                    ...
                    if(aBound && Array.isArray(aBound) && aBound.length > 0){
                        fnAfter = function(){
                            let oFirst = aBound[0];
                            parent.IPCRENDERER.send(`${oContr.IF_DATA.BROWSKEY}--if-ui5css`,
                                { PRCCD: "SELECT_UI", OBJID: oFirst.OBJID, UIATK: oFirst.UIATK });
                        };
                    }

                    if(sErrMsg !== ""){

                        let oOpt = fnAfter ? { onClose: fnAfter } : {};

                        if(oRES.MSGTY === "E"){
                            sap.m.MessageBox.error(sErrMsg, oOpt);
                        }else{
                            sap.m.MessageBox.warning(sErrMsg, oOpt);
                        }
