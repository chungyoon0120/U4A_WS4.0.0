...
(function (window, $, oAPP) {
    ...
    function _buildWs20Split() {
        ...
        LEFT.style.flex = "0 1 25%";     // 초기 basis=%(반응형). 드래그하면 공통 wireSplitter 가 0 0 <px> 로 고정.
        ...
        // 창 리사이즈 재클램프는 공통 wireSplitter 가 _ws20ArrangeSplit 에서 자동 등록(별도 배선 불필요, 16 §4.4).
    ...
    oAPP.fn.fnWs20OpenLayoutPopup = function () {
        ...
        function _renderCards() {
            ...
            aWork.forEach(function (sid) {
                ...
                C.addEventListener("drop", function (e) {
                    ...
                    // ★[BR60-1] 원본(callDesignLayoutChangePopup.js lf_drop 299~307행)은 끌던 타일과
                    //   놓인 타일의 '자리를 서로 맞바꾼다'(사이에 낀 가운데 타일은 그대로). 이전 변환본은
                    //   끌던 타일을 빼서 놓은 자리에 끼워넣어(밀어넣기) 한 칸 건너 옮길 때 결과가 원본과
                    //   달랐다. 예) [트리·미리보기·속성]에서 트리→속성 위로: 원본=[속성·미리보기·트리],
                    //   밀어넣기=[미리보기·속성·트리]. 원본대로 맞바꿈으로 복원.
        ...
        function _save() {
            ...
            function _doSave() {
                ...
                //   + setShortcutLock(true)(253행). 잠금은 미리보기 재로드가 끝난 뒤에만 푼다(WP1 — 중간에
                //   풀면 그 틈에 연타/단축키가 들어와 상태가 꼬임).
                //
                //   [BR60 검수반영] 화면잠금·단축키잠금은 **소유권 단위로 각각 한 번만** 푼다.
                //   둘 다 참조 카운트 없는 전역 상태라, 이 저장이 이미 놓은 잠금을 두 번 끄면
                //   그 사이 다른 작업이 새로 건 잠금까지 풀려 버린다(중복 해제 금지 = 아래 소유 표시).
                //   · 화면잠금 소유권은 "이 저장이 미리보기 재로드를 남에게 맡기면" 그쪽으로 넘긴다
                ...
                if (_bReparented) {
                    ...
                    try {
                        ...
                        if (_pf) {
                            // ★[BR60-3][BR60-4] 재부착으로 시작된 미리보기 재로드가 끝난 뒤에만 잠금 해제(WP1).
                            //   성공(load)·실패(error) 어느 쪽이든 반드시 한 번 회수해 화면이 잠긴 채 멈추지 않게 한다
                            //   (미리보기 로더의 iframe 오류 안전장치와 같은 방식 — ws_html5_ws20_prev.js).
                            var _offPrevLd = function () {
                    ...
                    // ★[BR60-3][BR60-4] 이번 저장은 스스로 재로드를 걸지 않았다 → 기다릴 완료 신호가 없으므로
                    //   이 저장이 건 잠금은 지금 회수한다(안 풀면 화면이 잠긴 채 멈춘다).
                    _unlockAll();
                } else {
                    // 순서 무변경 + 진행 중 재로드 없음 → 안정 컨텍스트 → 예전처럼 명시 재로드로 미리보기 갱신.
                    // ★[BR60-3][BR60-4] 미리보기 로더는 자기 화면잠금을 걸고 **미리보기 구성이 끝난 뒤**
                    //   스스로 푸는 계약을 이미 갖고 있다(ws_html5_ws20_prev.js 의 로드 시작/종료 처리).
                    //   → 화면잠금 소유권을 로더에 넘기고(여기서 끄면 로더가 방금 건 잠금까지 꺼져
                    //   그리기 도중 화면이 열린다), 로더가 관리하지 않는 단축키 잠금만
                    //   "미리보기 로드 종료" 알림을 받아 그때 푼다(WP1 직렬화).
                    ...
                    if (typeof oAPP.fn.fnWs20LoadPreview === "function") {
                        try {
                            ...
                            oAPP.attr.__ws20PrevDoneCbs.push(_releaseShortcut);
                            _bOwnBusy = false;                 // 화면잠금 소유권 이전(로더가 해제).
                    ...
                    // 로더가 없거나 호출이 실패했으면 기다릴 완료 신호가 없다 → 즉시 회수.
                    if (!_bDelegated) { _unlockAll(); }
                }
                _close(true);   // ★[BR60-2] 저장으로 닫히는 경로 → 취소 안내 없음(원본 lf_after→lf_close(_, true)).
            }
            try {
