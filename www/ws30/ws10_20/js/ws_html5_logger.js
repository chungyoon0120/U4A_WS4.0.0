...
(function (global) {
    ...
    function _detectWindowName() {
        ...
        if (!sTop) {
            try { sTop = document.title || ''; } catch (e) { if (typeof U4ALOG !== "undefined" && U4ALOG.caught) { U4ALOG.caught(e); } }
        }
    ...
    function _detectScreenName() {
        ...
        try { s = document.title || ''; } catch (e) { if (typeof U4ALOG !== "undefined" && U4ALOG.caught) { U4ALOG.caught(e); } }
    ...
    function _buildLine(sLevel, sWhat, sTarget, sResult, iElapsedMs, bWithGap, bNoAt) {
        ...
        if (!_sWindowName) {
            try { _sWindowName = _detectWindowName(); } catch (e) { if (typeof U4ALOG !== "undefined" && U4ALOG.caught) { U4ALOG.caught(e); } }
        }

        if (!_sScreenName) {
            try { _sScreenName = _detectScreenName(); } catch (e) { if (typeof U4ALOG !== "undefined" && U4ALOG.caught) { U4ALOG.caught(e); } }
        }
    ...
    var U4ALOG = {
        ...
        caught: function (oError, sWhere) {
            ...
            try {
                ...
                /**
                 * 먼저 몇 번째인지부터 센다 (2026-09-08 보완).
                 * 앞서는 남기지도 않을 것의 자리를 매번 뽑느라 느렸다.
                 * 세는 열쇠는 오류 내용 — 같은 내용이면 대개 같은 자리다.
                 */
                var sKey = (oError && oError.message)
                    ? String(oError.message)
                    : String(sWhere || oError || '');
    ...
    /**
     * 화면에 반드시 있어야 하는 요소를 못 찾았을 때 (2026-09-08 추가 — 장군님 지시)
     * -------------------------------------------------------------------
     * 왜 넣었나
     *   화면 요소를 찾아 놓고 있는지 안 보고 바로 쓰는 자리가 103곳 있다(실측).
     *   없으면 그 자리에서 터지는데 "무엇을 못 찾았는지" 가 로그에 안 남는다.
     *
     * 어떻게 하나
     *   요소를 찾는 공통 기능을 감싼다. 화면마다 손대지 않으므로 빠짐이 없다.
     *   원래 동작은 그대로 — 못 찾으면 원래대로 빈 값을 돌려준다. 흐름을 막지 않는다.
     *
     * 로그가 터지지 않게
     *   같은 이름을 계속 못 찾는 것은 처음 3번만 남기고 그 뒤엔 정해진 횟수마다.
     */
    var _oMissCount = {};

    function _logMiss(sHow, sWhat) {
    ...
    /** 요소 찾는 공통 기능을 감싼다 — 원래 동작 그대로, 못 찾을 때만 한 줄 남긴다 */
    function _wrapDomFinders() {

        try {

            if (!global.document || global.document.__u4aFindWrapped) {
                return false;
            }
            ...
            if (typeof d.getElementById === 'function') {
                ...
                var fnById = d.getElementById.bind(d);

                d.getElementById = function (sId) {

                    var el = fnById(sId);

                    if (!el) { _logMiss('id', sId); }
            ...
            if (typeof d.querySelector === 'function') {
                ...
                var fnSel = d.querySelector.bind(d);

                d.querySelector = function (sSel) {
