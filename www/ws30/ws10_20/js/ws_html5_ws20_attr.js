...
(function (window, $, oAPP) {
    ...
    //UNDO(되돌리기) 이력에 안 쌓는 액션코드 — 원본 uiAttributeArea.js:29~35 CT_SKIP_UNDO_ACTION 1:1.
    //  (집합 바인딩 해제·재바인딩, 트리 부모/자식 키 해제, 클라이언트 이벤트 추가·삭제, HTML content 편집)
    var CT_SKIP_UNDO_ACTION = ["ADD_CLIENT_EVENT", "DEL_CLIENT_EVENT", "UNBIND_AGGR", "UNBIND_TREE_KEY", "HTML_CONTENT_EDITOR"];
    ...
    /************************************************************************
     * [BR59-4] 되돌리기 기록에 실을 "이번에 값이 바뀐 속성" 정보.
     *   원본 undoRedo CL_CHANGE_ATTR 2278 은 되돌리기/다시하기 후
     *   `setSelectTreeItem(T_ATTR[0].OBJID, T_ATTR[0].UIATK)` 로 **값이 바뀐 그 UI 를 선택하고
     *   그 속성 줄까지** 찾아간다. 그동안 HTML5 는 아무것도 안 넘겨 이 동작이 통째로 빠져 있었다.
     *   (속성 행에 OBJID 가 없을 때는 지금 오른쪽에 떠 있는 UI 로 보정.)
     ************************************************************************/
    ...
    function _ensureODesign() {
        try {
            ...
            //attrChange 액션코드 — 원본 uiAttributeArea.js:20~26 CS_ACTCD 1:1.
            //  (일부 바인딩 조작에 붙여 UNDO 이력 생략 판단에 쓴다. 위 CT_SKIP_UNDO_ACTION 과 짝.)
            //  ★원본은 uiAttributeArea.js 에서 정의했으나 그 파일은 UI5 의존이라 HTML5 미로드 → 여기서 동일 값 구성.
            if (!oAPP.oDesign.CS_ACTCD) {
                oAPP.oDesign.CS_ACTCD = {
                    ...
                    UNBIND_TREE_KEY: "UNBIND_TREE_KEY",         //TREE 의 PARENT, CHILD 바인딩 해제.
                    HTML_CONTENT_EDITOR: "HTML_CONTENT_EDITOR" //sap.ui.core.HTML 의 content 프로퍼티.
                };
            }
        } catch (e) {
    ...
    //Description 삭제. [BR40] 원본 uiAttributeArea.js:7871 1:1 이식 복원 (배선은 ws_html5_ws20_edit.js 삭제 2경로에 존재)
    if (typeof oAPP.fn.delDesc !== "function") {
        oAPP.fn.delDesc = function (OBJID) {

            // guard: T_DESC store not ready yet (called before load) -> description cannot be removed
            if (!oAPP.DATA || !oAPP.DATA.APPDATA || !oAPP.DATA.APPDATA.T_DESC) {
                if (typeof U4ALOG !== "undefined" && U4ALOG.warn) { U4ALOG.warn("GUARD_EXIT", "T_DESC missing", "skip delDesc(" + OBJID + "), description not removed"); }
                return;
            }
            ...
            //못찾은경우 EXIT. (already removed / no description) - normal no-op
            if (l_indx === -1) { return; }
    ...
    if (typeof oAPP.fn.attrChnageOBJID !== "function") {
        oAPP.fn.attrChnageOBJID = async function () {
            ...
            //[BR62-A] 되돌리기 대상 = **되돌린 뒤의 이름**(= 바꾸기 전 옛 이름).
            //  여기서 대상을 안 남기면, 복원 시 앵커가 "그때 선택돼 있던 이름"으로 잡히는데
            //  그 값은 이미 **새 이름**이라 되돌린 트리에 없다 → 부모(예: PAGE1)로 선택이 튄다.
            //  옛 이름을 남기면 되돌리기는 그 줄을 그대로 찾고, 다시하기는 옛 이름이 없어
            //  기존 폴백(그때 선택 = 새 이름)이 걸려 **양방향 모두 같은 UI**가 선택된다.
            //  ★UIATK 는 주지 않는다 — 원본 이름변경 되돌리기(undoRedo.js CL_CHANGE_OBJID 1405~1560)에는
            //    속성 줄로 이동하는 처리가 없다(전 구간 setSelectTreeItem 부재).
            var _oRenameSnap = null;
            try {
            ...
            //★[BR62-이름변경] 이 편집이 "이름만 바꿈"임을 되돌리기 기록에 새긴다.
            //  이게 없으면 복원 때 스냅샷 비교가 옛 이름/새 이름을 **서로 다른 UI**로 보고
            //  미리보기를 destroy 후 재생성하고(속성이 바뀐 것도 아닌데), 트리 펼침 기억도
            //  옛 이름으로 못 돌아가 하위가 저절로 펼쳐진다(장군님 지적 2026-09-02).
            //  원본은 인스턴스를 그대로 두고 키만 교체한다(undoRedo.js CL_CHANGE_OBJID 1450~1491).
            try {
            ...
            if (l_tree) {
                ...
                //[BR59-6] 디자인 트리의 "선택된 줄" 표시는 HTML5 가 **UI 이름으로** 찾아 칠한다
                //  (원본 UI5 는 트리 줄 자체를 붙잡고 있어 이름이 바뀌어도 선택이 안 풀린다).
                //  이름만 바꾸고 이 선택 이름을 안 옮기면, 바뀐 뒤 짝이 맞는 줄이 없어져
                ...
                try {
                    ...
                    if (oExp && Object.prototype.hasOwnProperty.call(oExp, sBf)) {
                        oExp[sNew] = oExp[sBf];
                        delete oExp[sBf];
                    }
    ...
    // ★ 선택 테두리가 안 뜬 경우 다시 그려달라고 요청(선택 직후 짧은 구간만).
    //   증상: 글자편집기(RichTextEditor)를 고르면 테두리가 아예 안 뜬다.
    //   원인(CDP 실측): 선택표시 함수가 대상에 포커스를 준 "직후" 곧바로 테두리를 계산하는데,
    //   그 순간엔 화면 이동이 아직 반영되지 않아 대상이 화면 밖으로 판정된다 → 테두리를 숨기고
    //   지켜보기(감시)까지 끊어, 이동이 끝난 뒤에도 아무도 다시 그려주지 않는다.
    //   (실측: 선택 직후 테두리 없음 → 다시 그려달라고 한 번만 요청하면 695x200 정상 표시)
    //   편집기의 "준비 완료" 신호는 이미 그려진 편집기에선 다시 오지 않아 기다리는 방식은 불가(실측 확인).
    //   그래서 뜰 때까지 몇 프레임만 다시 그려달라고 요청한다. 뜨면 즉시 종료, 선택이 바뀌면 중단.
    //   ※ 미리보기에 "다시 그려달라"고만 한다 — 글자편집기 엔진은 건드리지 않는다(오류 재발 방지).
    function _reassertPreviewSelection(OBJID) {
        try {
            var oWin = _frameWin();
            if (!oWin || !oWin.oWS || !oWin.oWS.sMark) { return; }
            if (typeof oWin.requestAnimationFrame !== "function") { return; }
            if (typeof oWin.requestPreviewSelectionLayerUpdate !== "function") { return; }
            var doc = oWin.document, sMark = oWin.oWS.sMark;
            var iFrames = 0, iStable = 0;
            var MAX_FRAMES = 60;   // ~1초 상한
            function _tick() {
                iFrames++;
                try {
                    // 선택이 다른 UI 로 바뀌었으면 손대지 않는다.
                    if (!sMark.oSelectedUi || sMark.oSelectedUi._OBJID !== OBJID) { return; }
                    if (doc.querySelector('[data-u4a-layer-visible]')) {
                        if (++iStable >= 3) { return; }   // 떴고 유지됨 → 종료
                    } else {
                        iStable = 0;
    ...
    // ★ 화면(Page 등) 자체를 고르면 미리보기를 그 화면의 처음(위쪽)이 보이게 맞춘다.
    //   원본 선택표시는 "포커스 주기"로 화면을 옮기는데, 이 방식은 필요한 만큼만 움직인다.
    //   화면(Page)은 보이는 영역보다 커서 이미 영역을 꽉 채우므로 한 칸도 안 움직이고,
    //   그 전에 아래쪽 UI(예: 글자편집기)를 골라 내려가 있던 위치가 그대로 남아
    //   화면 위쪽(달력 등)이 안 보인 채로 남는다(CDP 실측: 편집기 선택 후 스크롤 716,
    //   PAGE1 을 다시 골라도 716 유지 → 위쪽 달력 안 보임).
    //   그래서 "고른 UI가 보이는 영역보다 큰 경우"에 한해 그 위쪽 끝에 맞춰 준다.
    //   ※ 스크롤 값만 한 번 바꾼다 — 미리보기 UI·글자편집기 엔진은 건드리지 않는다.
    function _alignPreviewToScreenTop(OBJID) {
        try {
            var oWin = _frameWin();
            if (!oWin || !oWin.document) { return; }
            var oUi = oAPP.attr.prev && oAPP.attr.prev[OBJID];
            var oDom = (oUi && typeof oUi.getDomRef === "function") ? oUi.getDomRef() : null;
            if (!oDom || typeof oDom.getBoundingClientRect !== "function") { return; }

            // ① 이 UI 를 담고 있는 바깥 스크롤 영역 먼저 찾는다(낱개 UI 인 경우).
            var oOuter = oDom.parentElement;
            while (oOuter) {
                if (oOuter.scrollHeight - oOuter.clientHeight > 2) { break; }
                oOuter = oOuter.parentElement;
            }
            if (oOuter) {
                // 고른 UI 가 보이는 영역보다 작으면 = 낱개 UI → 원본 동작(필요한 만큼만 이동) 유지.
                if (oDom.getBoundingClientRect().height < oOuter.clientHeight) { return; }
                var iDelta = Math.round(oDom.getBoundingClientRect().top - oOuter.getBoundingClientRect().top);
                if (iDelta === 0) { return; }
                oOuter.scrollTop = oOuter.scrollTop + iDelta;
                return;
            }

            // ② 바깥에 스크롤 영역이 없으면 = 고른 것이 화면(Page) 자체다.
            //    화면은 스크롤 영역을 자기 "안"에 품으므로, 그 안쪽 내용을 맨 위로 되돌린다.
            //    (CDP 실측: 편집기 선택으로 716 까지 내려간 상태 → 0 으로 복귀 → 위쪽 달력 다시 보임)
            var aInner = oDom.querySelectorAll("*");
            var oInner = null, iMax = 2;
            for (var i = 0; i < aInner.length; i++) {
                var iGap = aInner[i].scrollHeight - aInner[i].clientHeight;
                if (iGap > iMax) { iMax = iGap; oInner = aInner[i]; }
            }
    ...
    /************************************************************************
     * [BR53] 속성 목록의 스크롤이 멎을 때까지 기다린다.
     *   오류 줄로 이동시킨 직후 곧바로 콤보 펼침목록을 열면, 공통 콤보의 "바깥이 스크롤되면 목록을
     *   닫는" 규칙(theme/u4a-ui.js createSelect)에 걸려 방금 만든 이동 때문에 스스로 닫힌다.
     *   실제 스크롤 위치를 프레임마다 읽어 더 안 움직이면 끝난 것으로 본다(시간 때우기 아님).
     *
     *   [BR53-2차 / 코덱스·안티 P2-2] 프레임 카운터만으로는 상한이 보장되지 않는다 — 창을 최소화하거나
     *   가려 두면 프레임 콜백이 안 돌아 카운터가 멈추고 이 약속이 영영 안 끝난다. 이 함수가 안 끝나면
     *   아래 호출부가 걸어둔 화면잠금까지 영구히 남으므로, 실제 경과시간 타이머(_MAXMS)를 함께 두어
     *   어느 환경에서든 반드시 한 번은 끝나게 한다(먼저 도달한 쪽이 끝냄, 나머지는 정리).
     ************************************************************************/
    function _attrWaitScrollIdle() {
        var _MAXMS = 800;   //부드러운 이동이 이보다 오래 걸리는 경우는 없다(상한 보장용).
        return new Promise(function (res) {
            ...
            var SC = document.getElementById("ws20AttrScroller");
            if (!SC) { requestAnimationFrame(function () { requestAnimationFrame(fin); }); return; }
            var iLast = SC.scrollTop, iSame = 0, iTick = 0;
            (function step() {
                requestAnimationFrame(function () {
                    if (bDone) { return; }
                    iTick++;
                    if (SC.scrollTop === iLast) { iSame++; } else { iSame = 0; iLast = SC.scrollTop; }
                    if (iSame >= 3 || iTick >= 40) { fin(); return; }
                    step();
                });
            })();
        });
    ...
    if (typeof oAPP.fn.setAttrFocus !== "function") {
        oAPP.fn.setAttrFocus = function (UIATK, TYPE, oOpt) {
            ...
            try {
                ...
                //type에 따른 value state 처리. (원본 8246~8267행 1:1 — E/S/I/W 4종)
                switch (TYPE) {
                    case "E": sAttr.valst = "Error"; break;
                    case "S": sAttr.valst = "Success"; break;
                    case "I": sAttr.valst = "Information"; break;
                    case "W": sAttr.valst = "Warning"; break;
                    default: break;
                }
                ...
                if (oRow) {
                    ...
                    //  input/select/textarea/button 뿐이라, 값 컨트롤이 콤보(div)인 DDLB 라인에서는
                    //  대상을 못 찾아 포커스가 통째로 빠졌다. 못 찾으면 종전대로 행을 이동시킨다.
                    var oTgt = oRow.querySelector(
                        ".u4aWs20AttrRowVal input, .u4aWs20AttrRowVal textarea,"
                        + " .u4aWs20AttrRowVal select, .u4aWs20AttrRowVal .u4a-combo,"
                        + " .u4aWs20AttrRowVal button");

                    //DOM 위치로 부드러운 스크롤 이동 — 원본 8346행 1:1.
                    try {
    ...
    oAPP.fn.fnWs20SelectUI = async function (OBJID, oOpt) {
        ...
        try {
            ...
            if (oWin && oNode && typeof oWin.selPreviewUI === "function") {
                ...
                // 화면(Page 등) 자체를 고른 경우 그 화면 처음이 보이도록 맞춤(낱개 UI 는 원본 동작 유지).
                _alignPreviewToScreenTop(OBJID);
                // 테두리가 안 뜬 경우(글자편집기 등) 뜰 때까지 짧게 다시 그려달라고 요청.
    ...
    /************************************************************************
     * autoGrowing 프로퍼티 값에 따른 예외처리 — 원본 uiAttributeArea.js
     *   attrSetAutoGrowingException(3237행) 1:1 이식. [BR29]
     *   · sap.ui.table.Table(EXT00001347) → firstVisibleRowChanged(AT000013085)
     *   · sap.m.Table(EXT00001348)       → growing/update Started/Finished 4종
     *   · sap.m.List(EXT00001349)        → growing/update Started/Finished 4종
     *   autoGrowing=true 면 대상 이벤트를 입력불가(edit=false)+서버/클라 이벤트 아이콘
     *   숨김(icon1/2_visb=false)으로 잠근다. bClear=true 면 등록된 서버/클라 이벤트까지
     *   초기화(UIATV/ADDSC="" + attrDelClientEvent)한다.
     *   ★ 원본 undo(saveActionHistoryData)는 HTML5 스냅샷 스택(fnWs20PushUndo)으로 통일 →
     *     호출부가 변경 직전 1회 적재하므로 여기선 생략. oModel.refresh()→fnRenderWs20AttrRows().
     * @param {object}  is_attr        autoGrowing attr 라인(없으면 현재 UI 기준 자동 검색)
     * @param {boolean} bModelRefresh  true 면 처리 후 속성 행 재렌더
     * @param {boolean} bClear         true 면 대상 이벤트 초기화까지 수행
     ************************************************************************/
    if (typeof oAPP.fn.attrSetAutoGrowingException !== "function") {
        oAPP.fn.attrSetAutoGrowingException = function (is_attr, bModelRefresh, bClear) {
            ...
            //입력 attr 이 없으면 현재 출력된 UI 기준으로 autoGrowing attr 검색(원본 3239).
            if (!is_attr) {
                var sUIOBK = (oAPP.attr.oModel && oAPP.attr.oModel.oData
                    && oAPP.attr.oModel.oData.uiinfo && oAPP.attr.oModel.oData.uiinfo.UIOBK) || "";
                switch (sUIOBK) {
                    case "UO00326": is_attr = aAttr.find(function (a) { return a.UIATK === "EXT00001349"; }); break; //sap.m.List
                    case "UO00447": is_attr = aAttr.find(function (a) { return a.UIATK === "EXT00001348"; }); break; //sap.m.Table
                    case "UO01139": is_attr = aAttr.find(function (a) { return a.UIATK === "EXT00001347"; }); break; //sap.ui.table.Table
                    default: return; //autoGrowing 처리 대상 UI 아님(원본 3257).
                }
            }

            //autoGrowing attr 를 찾지 못한 경우 exit(원본 3263).
            if (!is_attr) { return; }

            //autoGrowing KEY 별 점검대상 이벤트 attr key 구성(원본 3268).
            var lt_UIATK = [];
            switch (is_attr.UIATK) {
                case "EXT00001347": lt_UIATK = ["AT000013085"]; break;                                          //Table firstVisibleRowChanged
                case "EXT00001348": lt_UIATK = ["AT000005916", "AT000005917", "AT000005918", "AT000005919"]; break; //sap.m.Table
                case "EXT00001349": lt_UIATK = ["AT000003866", "AT000003867", "AT000003868", "AT000003869"]; break; //sap.m.List
                default: return; //autoGrowing 이 아닌 경우 exit(원본 3284).
            }

            //autoGrowing 값이 true 면 대상 이벤트 입력 불가(원본 3304~3311).
            var l_edit = (is_attr.UIATV !== "true");

            //점검대상 이벤트 항목 처리(원본 3313).
            for (var i = 0, l = lt_UIATK.length; i < l; i++) {

                var sKey = lt_UIATK[i];
                var ls_attr = aAttr.find(function (a) { return a.UIATK === sKey; });

                //대상 이벤트를 찾지 못한 경우 skip(원본 3320).
                if (typeof ls_attr === "undefined") { continue; }

                //입력 가능여부 + 서버/클라 이벤트 아이콘 표시여부(원본 3323~3329).
                ls_attr.edit = l_edit;
                ls_attr.icon1_visb = l_edit;
                ls_attr.icon2_visb = l_edit;

                //초기화 처리가 아니면 skip(원본 3332).
                if (bClear !== true) { continue; }

                //서버이벤트 입력값 + 클라이언트 SOURCE TYPE 초기화(원본 3335~3338).
                ls_attr.UIATV = "";
                ls_attr.ADDSC = "";
    ...
    /************************************************************************
     * autoGrowing 프로퍼티 변경건 예외처리 — 원본 uiAttributeArea.js
     *   attrChangeAutoGrowingProp(3115행) 1:1 이식. [BR29]
     *   sap.ui.table.Table / sap.m.Table / sap.m.List 의 autoGrowing 을 true 로
     *   바꾸면 "이전 서버/클라 이벤트가 초기화됩니다. 진행?"(메시지 283) 확인 팝업 →
     *   YES 시 대상 이벤트(예: firstVisibleRowChanged) 초기화 + 입력잠금.
     *   ★ HTML5: 원본 attrChangeProc(값 반영 본문)는 fnWs20AttrChange 로 대체됨 →
     *     확인 콜백은 이벤트 예외처리 후 fnWs20AttrChange 를 재사용한다
     *     (bSkipUndo=true: 스냅샷 1회 이미 적재 / bSkipAutoGrow=true: 팝업 재호출 방지).
     * @param {object} is_attr - 이벤트 발생 attr 라인.
     * @return {boolean} true = 호출부(fnWs20AttrChange) 후속 처리 skip.
     ************************************************************************/
    if (typeof oAPP.fn.attrChangeAutoGrowingProp !== "function") {
        oAPP.fn.attrChangeAutoGrowingProp = function (is_attr) {

            //autoGrowing 프로퍼티 변경건이 아닌경우 미해당(원본 3117).
            ...
            //바인딩 처리된경우 exit(원본 3124).
            if (is_attr.ISBND === "X") { return; }

            //autoGrowing 을 true 로 설정하지 않은경우 — 대상 이벤트 잠금 해제 후
            ...
            //autoGrowing 을 true 로 설정한 경우 확인 팝업 호출(원본 3146).
    ...
    /************************************************************************
     * [BR54] attribute 값 변경 전용 예외처리 — 원본 design/attributesArea/attrChangeException.js
     *   (원본 uiAttributeArea.js attrChange 1840~1845 행이 호출) 1:1 이식.
     *
     *   원본 동작 3단계:
     *     (1) 패치 UHAK901369 가 안 깔려 있으면 아무것도 안 한다 → 정규 값 처리 그대로 진행.
     *     (2) 코드마스터 UW13 에 이 속성(FLD01 = 속성키)이 등록돼 있지 않으면 역시 아무것도 안 한다.
     *     (3) 등록돼 있으면 FLD04 + FLD05 경로의 전용 모듈을 그때 불러와 default(sAttr) 를 실행하고
     *         true 를 돌려준다 → 호출측(attrChange)이 이후 값 처리(attrChangeProc)를 통째로 건너뛴다.
     *
     *   ★ HTML5 에서 판정/실행을 둘로 쪼갠 이유:
    ...
    function _isAttrChangeException(sAttr) {
        ...
        if (!oAPP.common.checkWLOList("C", "UHAK901369")) { return null; }
        ...
        return oAPP.attr.S_CODE.UW13.find(function (item) {
            return item.FLD01 === (sAttr && sAttr.UIATK);
        }) || null;
    ...
    /************************************************************************
     * fnWs20AttrChange 가 진입부에서 건 잠금(로딩표시·단축키·자식창·재진입)을 한꺼번에 푼다.
    ...
    oAPP.fn.fnWs20AttrChange = function (sAttr, uityp, bSkipUndo, bSkipAutoGrow, bSkipAttrExc) {
        ...
        //[BR54] 전용 예외처리 모듈에 넘긴 경우 true. 이때는 아래 finally 가 잠금을 풀지 않고,
        ...
        try {
            ...
            //  초기화는 팝업 콜백이 fnWs20AttrChange 재진입(bSkipAutoGrow=true)으로 수행한다.
            //  early return → 아래 finally 가 busy·단축키·자식창 잠금을 원복(원본 1787~1792 대응).
            if (bSkipAutoGrow !== true && typeof oAPP.fn.attrChangeAutoGrowingProp === "function"
                && oAPP.fn.attrChangeAutoGrowingProp(sAttr) === true) {
                return;
            }
            ...
            //  ★ 액션코드가 UNDO 생략 목록에 있으면 스냅샷을 안 쌓는다(원본 uiAttributeArea.js:1809 1:1).
            ...
            //  대상 속성이면 전용 모듈이 값 처리를 대신하므로, 아래 정규 처리는 통째로 건너뛴다(원본 1843~1845행 return).
            ...
            // 원본은 try/catch 없이 무조건 호출한다 — 실패하면 그대로 크게 드러난다.)
            try {
                var _oDesignChkModule = parent.require(
                    parent.PATH.join(oAPP.oDesign.pathInfo.designRootPath, "js", "checkAppData", "designTreeData.js"));
                ...
                //  변경 수집(prev._T_0015)·저장 후보에 넣지 않도록 후속 처리를 중단하고 종료.
                //  아래 finally 가 busy·단축키·자식창 잠금·재진입 플래그를 원복한다(autoGrowing 조기 return 과 동일 패턴).
                return;
            }
            ...
            if (_bExcPending !== true) { _releaseAttrChangeLock(); }
        }
    ...
    if (typeof oAPP.fn.attrAppF4Del !== "function") {
        oAPP.fn.attrAppF4Del = function (is_attr) {
            ...
            try {
                ...
                if (typeof oAPP.fn.fnWs20PushUndo === "function") { oAPP.fn.fnWs20PushUndo(_undoFocus(is_attr)); }
    ...
    /************************************************************************
     * dropAble 프로퍼티 입력값에 따른 DnDDrop 이벤트 예외처리 — [BR31]
     *   원본 uiAttributeArea.js attrSetDropAbleException(3388행) 이식.
     *   · dropAble=true(또는 바인딩) → DnDDrop 이벤트 입력 가능(잠금 해제).
     *   · dropAble≠true → DnDDrop 이벤트 입력 잠금(edit=false). bClear=true 이면
     *     등록된 DnDDrop 서버/클라이언트 이벤트까지 초기화(_T_0015 라인 제거).
     *   · 특정 UI/OBJID 가 아니라 UIASN(DROPABLE/DNDDROP) 의미명으로 판단하므로
     *     dropAble+DnDDrop 을 함께 제공하는 모든 UI 에 동일 적용(원본 동일, BR31 요구).
     *   소비처: (1) 렌더 _updateAttrList 끝(무인자) — 현재 dropAble 값으로 DnDDrop 잠금만.
     *          (2) 값 변경 fnWs20AttrChange 의 dropAble 변경 훅(is_attr, false, true) — 잠금+초기화.
     *   HTML5 차이(원본 대비):
     *     · UI5 전용 undoRedo.saveActionHistoryData 생략 — 변경 경로(fnWs20AttrChange)가 이미
     *       fnWs20PushUndo 로 _T_0015 스냅샷을 1회 적재하므로 DnDDrop 초기화도 함께 undo 된다.
     *     · 원본이 dropAble 라인 자체를 attrChangeProc 로 재수집하던 부분 생략 — fnWs20AttrChange 가
     *       dropAble 라인 수집(attrChgAttrVal)을 이미 마친 뒤 이 함수를 부른다(이중수집 방지).
     *     · UI5 oModel.refresh() → HTML5 fnRenderWs20AttrRows(). 단 두 소비처 모두 호출처가
     *       재렌더/바인딩팝업 반영을 담당하므로 bModelRefresh 는 false 로 넘겨 데이터/edit 만 갱신.
     ************************************************************************/
    if (typeof oAPP.fn.attrSetDropAbleException !== "function") {
        oAPP.fn.attrSetDropAbleException = function (is_attr, bModelRefresh, bClear) {
            ...
            if (typeof is_attr === "undefined") {
                is_attr = aAttr.find(function (a) { return a.UIASN === "DROPABLE"; });
            }

            //dropAble 프로퍼티가 없거나 변경건이 아닌경우 exit.
            ...
            //dropAble 값이 true 인경우 drop 이벤트 입력 가능.
            if (is_attr.UIATV === "true") { l_edit = true; }

            //바인딩 처리된 경우에도 drop 이벤트 입력 가능.
            if (is_attr.UIATV !== "" && is_attr.ISBND === "X") { l_edit = true; }

            //선택한 값을 combobox value 필드에 매핑.(원본 동일)
            is_attr.comboval = is_attr.UIATV;

            //DnDDrop 이벤트 라인 찾기.
            var ls_drop = aAttr.find(function (a) { return a.UIASN === "DNDDROP"; });
            if (!ls_drop) { return; }
        ...
        };  //dropAble 프로퍼티 입력값에 따른 DnDDrop 이벤트 예외처리.
    }
    ...
    function _attrVsEl() {
        if (!_attrVsPop) {
            ...
            _attrVsPop.className = "u4a-field__msg u4aWs20AttrVsPop";
            _attrVsPop.setAttribute("data-vs", "error");
        }
        return _attrVsPop;
    }
    ...
    function _attrVsShow(oInputEl, sTxt) {
        ...
        if (!_attrVsBound) {
            ...
            document.addEventListener("mousedown", function (e) {
                ...
                var t = e.target;
                if (_attrVsField && t && t.closest && t.closest(".u4aWs20AttrValBox") === _attrVsField) { return; }
                if (_attrVsPop.contains && t && _attrVsPop.contains(t)) { return; }
                _attrVsHide();
            }, true);
        }
    }
    ...
    function _buildValueControl(sAttr) {
        ...
        if (sAttr.sel_visb === true) {
            ...
            // {value,text} 변환 (구 KEY/TEXT). 현재 값이 목록에 없으면 표시 보장용으로 맨 뒤에 붙인다.
            //   ★서버 목록의 선두 빈칸(KEY="")이 항상 최상위여야 하므로 선두(unshift)가 아니라 끝(push).
            var aItems = [];
            var bFound = false;
            ...
            if (!bFound && sVal) { aItems.push({ value: sVal, text: sVal }); }

            function _onPick(v) {
                sAttr.UIATV = v;
                sAttr.comboval = v;
                // [BR24] 값을 새로 고르면 이 라인의 오류표시 해제 — 유효한 서버이벤트를 선택하면 빨간 테두리가
                //   사라지게(원본 attrClearErrorField 의미). 프로퍼티 DDLB 는 이어지는 checkPropertyValue 가
                //   재검증하므로 여기서 지워도 안전.
                sAttr.valst = undefined; sAttr.valtx = undefined;
            ...
            // 콤보 공통 밸류스테이트(오류표시) 초기값 — 접힘 상태에서도 빨간 테두리·값 글자가 보이게 전달.
            //   프로퍼티 DDLB(checkPropertyValue 오류)·이벤트 DDLB 공통. 펼치면 목록 최상단에 문구가 뜬다.
            var oSelOpts = {
                valueState: (sAttr.valst === "Error") ? "error" : "none",
                valueStateText: sAttr.valtx || ""
            };
            ...
            if (sAttr.UIATY === "2" && !bDisabled) {
                oSelOpts.onOpen = function (oCombo) {
                    //[BR53] 미등록 이벤트라 그 줄로 이동시켰는지 표시 — 이동시켰다면 스크롤이 멎은 뒤
                    //  펼침목록을 연다(아래 참조).
                    var bMoved = false;
                    //[BR53-2차 / 코덱스·안티 P2-1] 이동 구간 화면잠금 소유 표시.
                    //  원본은 목록을 받은 뒤에도 잠금을 그대로 쥐고 있다가 "펼치기 바로 직전"에 푼다
                    //  (uiAttributeArea.js 1131~1148 = 잠금해제 후 _oUi.open(), F4 경로 1338~1344 동일).
                    //  HTML5 는 목록 조회 함수가 콜백 직전에 화면잠금을 먼저 풀어버려서
                    //  (getServerEventList 안 parent.setBusy("")), 이동·대기 동안 화면이 열려 있었다.
                    //  그 사이 다른 줄을 고르거나 변경항목 보기 버튼을 누르면 속성 줄이 통째로 다시 그려져,
                    //  사라진 값칸을 기준으로 펼침목록이 화면 구석에 떠버린다(두 검수 공통 지적).
                    //  → 이동을 시작할 때 화면잠금을 다시 걸고, 목록을 펼치기 직전에 푼다(원본과 같은 구간).
                    var bLocked = false;
                    // [BR24] 목록 누락 검증은 "조회 성공" 시에만 한다 — 원본은 성공 콜백 안에서만 누락을
                    //   판정한다(uiAttributeArea.js:1087-1113 은 success 경로). getServerEventList 는
                    //   서버 RETCD!=="S"·통신오류에는 콜백(f_cb)을 부르지 않고, 성공에만 f_cb(lt_ddlb) 를
                    //   부른다. 검증을 성공 콜백에 두면 통신/세션/백엔드 오류를 "목록에 없음"으로 오판해
                    //   정상 이벤트에 빨간표시를 다는 [코덱스 P1] 문제가 사라진다(실패 시 기존 상태 보존).
                    var fnOk = function (aDdlb) {
                        aDdlb = aDdlb || [{ KEY: "", TEXT: "", DESC: "" }];
                        sAttr.T_DDLB = aDdlb;   //서버이벤트 항목 매핑(원본 _sAttr.T_DDLB = _aEvent).
                        var aNew = [], bHas = false;
                        ...
                        // 등록된 서버이벤트가 조회 목록에 없으면 원본처럼 오류표시(원본 uiAttributeArea.js:1111
                        //   setAttrFocus(UIATK,"E")). 공통 콤보 밸류스테이트로 처리 → 접힘=빨간 테두리·글자,
                        //   펼침=목록 최상단 오류문구. 값이 목록에 없어도 콤보에 보이게 맨 뒤에 붙인다(선두 빈칸 유지).
                        if (!bHas && sAttr.UIATV) {
                            aNew.push({ value: sAttr.UIATV, text: sAttr.UIATV });
                            //[BR53] 원본 1:1 — 오류면 그 속성 줄로 이동·포커스한다
                            //  (uiAttributeArea.js:1129 setAttrFocus(_sAttr.UIATK, "E")).
                            //  이 경로는 "펼치기 직전"이라 행을 다시 그리면 지금 열리려는 콤보가 사라지므로
                            //  행 유지 모드로 부른다. setAttrFocus 는 원본대로(8244행) 오류표시 값과 안내문구를
                            //  먼저 비우고 오류표시만 세우므로, 안내문구는 그 뒤에 다시 채운다.
                            //이동 구간 화면잠금(원본 1:1) — 조회 함수가 콜백 직전에 이미 풀어놨으므로 다시 건다.
                            //  같은 처리 안에서 껐다 켜는 것이라 그 사이 그리기가 없어 깜빡임은 생기지 않는다.
                            try {
                            ...
                            sAttr.valst = "Error";
                            sAttr.valtx = oAPP.common.fnGetMsgClsText("ZMSG_WS_COMMON_001", "999", sAttr.UIATV || "", "", "", "");
                            oCombo.setValueState("error", sAttr.valtx);
                        } else {
                            //값이 유효(목록에 존재)하면 이 라인 오류표시 해제.
                            if (sAttr.valst === "Error") { sAttr.valst = undefined; sAttr.valtx = undefined; }
                            oCombo.setValueState("none");
                        }
                        oCombo.setItems(aNew);
                    };
                    // 반환 Promise 는 성공/실패 전 경로에서 resolve 되므로 드롭다운은 항상 열린다. 검증은 성공 시만.
                    return oAPP.fn.getServerEventList(fnOk, false).then(function () {
                        //[BR53] 오류 줄로 이동시켰다면 그 이동(스크롤)이 멎은 뒤에 펼침목록을 연다.
                        //  공통 콤보는 열린 뒤 바깥이 스크롤되면 목록을 닫으므로, 곧바로 열면 방금 만든
                        //  이동 때문에 목록이 스스로 닫혀 버린다.
                        if (!bMoved) { return; }
                        ...
                        //  여기로 오므로 잠금이 남지 않는다. 소유 표시로 한 번만 푼다(남의 잠금 보호).
                        if (!bLocked) { return; }
                        bLocked = false;
    ...
    /************************************************************************
     * [BR33] 서버이벤트 값칸 더블클릭 → 컨트롤러 클래스 메소드 이동.
     *   원본 uiAttributeArea.js:3089 attrDblClickServerEvent 1:1 이식.
     *   원본은 속성표 전체 더블클릭(attrDblclickEvent:2956)에서 행 정보를 읽어
     *   styleClass→bindField→serverEvent 순으로 분배한다. HTML5는 styleClass=F4,
     *   bindField=별도 경로로 이미 처리되고 서버이벤트 이동만 배선이 빠져 있었다.
     *   이벤트행(UIATY="2")에 서버이벤트값(UIATV)이 있으면 그 이름의 메소드로 이동.
     ************************************************************************/
    if (typeof oAPP.fn.attrDblClickServerEvent !== "function") {
        oAPP.fn.attrDblClickServerEvent = function (is_attr) {
            //이벤트가 아닌경우 EXIT. (원본 3092)
            if (!is_attr || is_attr.UIATY !== "2") { return; }
            //입력한 서버이벤트가 존재하지 않는경우 EXIT. (원본 3096 is_attr.UIATV === "" — null 안전 포함)
            if (!is_attr.UIATV) { return; }
            //trial 버전인경우 exit. (원본 3099)
            if (oAPP.fn.fnOnCheckIsTrial && oAPP.fn.fnOnCheckIsTrial()) { return; }
            //해당 이벤트로 네비게이션 처리. (원본 3102 execControllerClass(UIATV))
    ...
    oAPP.fn.fnRenderWs20AttrRows = function () {
        ...
        // [BR33] 서버이벤트 값칸 더블클릭 이동 — 목록 전체(ROWS)에 1회만 위임(원본 attrDblclickEvent 는
        //   속성표 레벨 단일 리스너). 값칸(.u4aWs20AttrRowVal) 안에서 발생한 더블클릭만 처리해
        //   이벤트명 링크·서버이벤트 생성 아이콘·클라이언트 이벤트 등록 아이콘(다른 셀)은 제외한다.
        //   ROWS 는 재사용 요소라 매 렌더마다 다시 붙지 않도록 가드.
        if (!ROWS.__bwpDblWired) {
            ROWS.__bwpDblWired = true;
            ROWS.addEventListener("dblclick", function (ev) {
                var oVal = ev.target && ev.target.closest ? ev.target.closest(".u4aWs20AttrRowVal") : null;
                if (!oVal) { return; }                                   // 값칸 밖(라벨·아이콘 셀)은 제외
        ...
        for (var i = 0, l = aAttr.length; i < l; i++) {
            ...
            // [BR24] 속성명 앞 종류 아이콘 — 원본 sap.m.ObjectStatus({text:UIATT, icon:UIATT_ICON}) 이식분.
            //   프로퍼티/이벤트/집합 라인 종류별 아이콘(attrSetLineStyle 에서 계산된 UIATT_ICON)을 라벨 앞에
            //   표시한다. 특히 이벤트에서 대기표시 미사용(ISWIT="X")이면 border→complete 로 바뀐 값이 여기서
            //   보인다. HTML5 이식 때 라벨의 icon 바인딩이 통째로 누락돼(텍스트만 그림) 표시가 안 되던 것.
            if (sAttr.UIATT_ICON) {
