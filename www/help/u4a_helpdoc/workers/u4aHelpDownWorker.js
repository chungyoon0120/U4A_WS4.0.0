...
function _getHelpDocuDataFromPowerShell(oPARAM){
    ...
    return new Promise(function(resolve){
        ...
        /**
         * HELP DOCUMENT 실행 파라미터 구성
         */
        let aParams = [
            `--PRCCD=UPGRADE`,                  // 프로세스 코드 (UPGRADE)
            `--ACTCD=u4a_help_doc_ws30`,        // 액션 코드 (u4a_help_doc_ws30)
            ...
            `--DOWN_PATH=${oPARAM.TMP_ROOT_PATH}`,       // [폴더] 저장 경로
            `--LOG_FLD_PATH=${oPARAM.LOG_FLD_PATH}`, // [폴더] 로그 저장 경로
            `--FILE_INFO=${JSON.stringify(FILE_INFO)}`        // 해더 메타 (JSON String)
        ];
        ...
        ps.stdout.on("data", (data) => {
            ...
            let sLog = `[Node Executer] Help Document 다운로드 중: ${data.toString()}`;
...
self.WS_HELP_DOCU_DOWN = async function (oPARAM) {    
    ...
    // 여기서 프로그래스바 Dialog를 닫지 않고 메인 스콥에서 닫을지 말지 판단한다!!
    // // 프로그래스바 Dialog Close
