(function(){
  ...
  oAPP.fn.uiAttributeArea = async function(oRPage){

    /**
     * @since   2026-01-06 17:10:25
     * @version v3.5.7-4
     * @author  pes
     * @description
     * UI의 onAfterRendering에서 DOM에 이벤트를 추가하는 module 로드.
     */
    let {attachExtendEvent} = await import(parent.PATH.join(oAPP.oDesign.pathInfo.designRootPath, 
        "attributesArea", "attachExtendEvent.js"));
    ...
    /**
     * @since   2026-03-24 12:04:30
     * @version v3.6.0-4
     * @author  pes
     * @description
     * UI명 출력 text에 Ctrl+ 마우스 우클릭을 통해 vscode로
     * 해당 source를 호출 하는 기능 추가.
     */
    parent.DEV_SOURCE_FINDER.setRunScriptPath(oRLibText);
     ...
     * @author  pes
     * @description
     * UI Sample 버튼에 Ctrl+ 마우스 우클릭을 통해 vscode로
     * 해당 source를 호출 하는 기능 추가.
     */
    parent.DEV_SOURCE_FINDER.setRunScriptPath(oRLibBtn2);
     ...
     * @author  pes
     * @description
     * OBJID 입력필드에 Ctrl+ 마우스 우클릭을 통해 vscode로
     * 해당 source를 호출 하는 기능 추가.
     */
    parent.DEV_SOURCE_FINDER.setRunScriptPath(oRInp1);
     ...
     * @author  pes
     * @description
     * OBJID 복사 버튼에 Ctrl+ 마우스 우클릭을 통해 vscode로
     * 해당 source를 호출 하는 기능 추가.
     */
    parent.DEV_SOURCE_FINDER.setRunScriptPath(oRBtn0);
     ...
     * @author  pes
     * @description
     * Description 입력 TextArea에 Ctrl+ 마우스 우클릭을 통해 vscode로
     * 해당 source를 호출 하는 기능 추가.
     */
    parent.DEV_SOURCE_FINDER.setRunScriptPath(oRTAr1);
     ...
     ************************************************************************/
    //attribute table UI.
    var oRTab1 = new sap.m.Table({mode:"SingleSelectMaster", 
      /**
       * @since   2025-12-31 18:11:58
       * @version v3.5.7-4
       * @author  pes
       * @description
       * attribute 출력 리스트에 변경된 라인을 css로 표현하는 과정에서
       * zebra 효과 때문에 변경된 라인 강조가 잘 보이지 않을 수 있기에
       * alternateRowColors를 주석 처리함.
       */
    ...
    /**
     * @since   2026-01-02 02:43:19
     * @version v3.5.7-4
     * @author  PES
     * @description
     * attribute 출력 테이블의 attr 변경 라인 표현을 위한 이벤트 등록 처리.
     */
    let {changedDataMarker} = await import(parent.PATH.join(oAPP.oDesign.pathInfo.designRootPath, "attributesArea", "changedDataMarker.js"));

    changedDataMarker(oRTab1);


    //drop UI 생성.
    var oDrop1 = new sap.ui.core.dnd.DropInfo({targetAggregation:"items", enabled:"{/IS_EDIT}"});
    oRTab1.addDragDropConfig(oDrop1);
    ...
    /**
     * @since   2026-01-28 14:50:05
     * @version v3.5.8-1
     * @author  pes
     * @description
     * attribute table의 toolbar 영역에 overflow toolbar 추가.
     * attribute의 기능이 추가 되는 경우 해당 overflow toolbar에 추가하여
     * 화면 크기가 작아져도 overflow 처리 되도록 함.
     * 메뉴 형식의 기능의 경우 sap.m.MenuButton UI를 활용하여
     * 하위 메뉴로 기능을 제공하도록 처리함.
     */
    let oOverTool = new sap.m.OverflowToolbar({
      width: "100%",
      style: "Clear"
    });
    oRTTool.addContent(oOverTool);
     ...
     * @author  pes
     * @description
     * attribute 초기화 버튼에 Ctrl+ 마우스 우클릭을 통해 vscode로
     * 해당 source를 호출 하는 기능 추가.
     */
    parent.DEV_SOURCE_FINDER.setRunScriptPath(oRTBtn1);
    ...
    oOverTool.addContent(new sap.m.ToolbarSpacer());
     ...
     * @author  pes
     * @description
     * UI의 attribute 변경건을 필터링 하는 버튼 추가.
     */   
    let _oFiltBtn = new sap.m.ToggleButton({
      pressed : "{/sAttrFilt/press}",
      text : "{/sAttrFilt/text}",
      //815 현재 UI에 설정된 Property, Event, Aggregation 항목 중 변경된 항목만 필터링하여 표시합니다. 기본값 상태의 항목은 목록에서 제외됩니다.
      tooltip : parent.WSUTIL.getWsMsgClsTxt("", "ZMSG_WS_COMMON_001", "815"),
      icon:"{/sAttrFilt/icon}",
      type: "Emphasized",
      // visible:"{/uiinfo/vis02}",
      // visible:"{/uiinfo/vis01}",
      press: async (oEvent)=>{
        ...
        const _oTab = oAPP.attr.ui.oRTab1;

        const _OBJID = oAPP.attr.oModel.getProperty("/uiinfo/OBJID");

        let _aT_0015 = oAPP.attr.prev?.[_OBJID]?._T_0015 || [];

        let {changedDataFilter, clearDataFilter, setButtonState} = await import(parent.PATH.join(oAPP.oDesign.pathInfo.designRootPath, 
          "attributesArea", "changedDataFilter.js"));

        //필터 버튼 text, icon 변경 처리.
        setButtonState({oUi:_oUi});


        //버튼 선택 해제 처리 되는 경우.
        if(_oUi.getPressed() === false){

          //필터 초기화.
          clearDataFilter({oUi:_oTab});
        ...
        //ui의 변경된 속성값 필터 처리.
        changedDataFilter({oUi:_oTab, T_0015: _aT_0015});
     ...
     * @author  pes
     * @description
     * 변경건 필터 버튼에 Ctrl+ 마우스 우클릭을 통해 vscode로
     * 해당 source를 호출 하는 기능 추가.
     */
    parent.DEV_SOURCE_FINDER.setRunScriptPath(_oFiltBtn);
    
    
    oOverTool.addContent(_oFiltBtn);
     ...
     * @author  pes
     * @description
     * UI attribute 개인화 항목을 확인하기 위한 버튼 추가.
     */
    let _oPresetList = new sap.m.OverflowToolbarButton({
      //652	UI Attribute Personalization List
      ...
      press : function(){
        parent.require(parent.PATH.join(oAPP.oDesign.pathInfo.designRootPath, 
          "attrPresetPopup", "index.js"))(parent.REMOTE, oAPP);

      }
    });
    oOverTool.addContent(_oPresetList);
   
    parent.DEV_SOURCE_FINDER.setRunScriptPath(_oPresetList);
     ...
     * @author  pes
     * @description
     * attr 도움말 팝업 호출 버튼에 Ctrl+ 마우스 우클릭을 통해 vscode로
     * 해당 source를 호출 하는 기능 추가.
     */
    parent.DEV_SOURCE_FINDER.setRunScriptPath(oRTBtn2);
     ...
     * @author  pes
     * @description
     * attr명 출력 UI에 Ctrl+ 마우스 우클릭을 통해 vscode로
     * 해당 source를 호출 하는 기능 추가.
     */
    parent.DEV_SOURCE_FINDER.setRunScriptPath(oRObjStat1);
     ...
     * @author  pes
     * @description
     * attr 입력필드에 Ctrl+ 마우스 우클릭을 통해 vscode로
     * 해당 source를 호출 하는 기능 추가.
     */
    parent.DEV_SOURCE_FINDER.setRunScriptPath(oRInp2);
    ...
    let _oExtEvent = new attachExtendEvent(oRSel1);

    _oExtEvent.attachEvent("click", async (e)=>{

      /**
       * @since   2026-01-29 02:17:10
       * @version v3.5.8-1
       * @author  PES
       * @description
       * combobox의 click 이벤트에서 서버이벤트 항목을 검색하는 로직보완 처리.
       * busy 처리 후 서버이벤트 항목을 검색하게 되는 경우 기존 table의 더블클릭 이벤트에서
       * 더블클릭한 위치가 이벤트 라인인 경우 SAP GUI를 호출하여 이벤트 메소드로 이동하는
       * 기존 기능이 동작하지 않기에 busy 처리 내용을 주석 처리함.
       * combobox의 click 이벤트시 lock 처리로 화면을 제어 하도록 로직 변경함.
       */
      // parent.setBusy("X");
      ...
      const _oEvent = e.getParameter("originalEvent");
      ...
      while (_oDom) {
        ...
        if (_oDom.id) {
          ...
          _oUi = sap.ui.getCore().byId(_oDom.id);
      ...
      //팝업의 펼침 아이콘을 선택한 경우 아이콘의 부모(combox) UI 얻기.
      if(_oUi?.isA?.("sap.ui.core.Icon") === true){
        _oUi = _oUi.getParent();

        //DDLB 아이콘을 선택하여 DDLB 항목 OPEN/CLOSE 처리함 FLAG.
        _isPopup = true;
      ...
      //UI의 바인딩된 라인 정보 얻기.
      let _sAttr = oAPP.oDesign.fn.getBindContextProperty(_oUi);
      ...
      //ROOT의 ATTR 항목인경우 EXIT.
      if(_sAttr.OBJID === "ROOT"){
      ...
      _oEvent.stopImmediatePropagation();

      //서버이벤트 항목 검색.
      const _aEvent = await new Promise((res)=>{
    ...
    //combobox의 dom에 keydown caback 이벤트 등록.
    _oExtEvent.attachEvent("keydown", async (e)=>{
      ...
      // 반복 입력 방지
      if (typeof _oEvent.repeat === "boolean" && _oEvent.repeat){
          return;  
      }

      let _key = _oEvent.key;
      let _oDom = _oEvent.target;


      //f4 펑션키를 누지 않은경우 exit.
      if(_key !== "F4"){
        //단축키도 같이 잠금 처리.
        oAPP.fn.setShortcutLock(false);
     ...
     * @author  pes
     * @description
     * attr 입력 DDLB에 Ctrl+ 마우스 우클릭을 통해 vscode로
     * 해당 source를 호출 하는 기능 추가.
     */
    parent.DEV_SOURCE_FINDER.setRunScriptPath(oRSel1);
    ...
    var oRBtn1 = new sap.m.Button({text:"{btn_text}", icon:"{btn_icon}", width:"100%", type:"{btn_type}", visible:"{btn_visb}"});
     ...
     * @author  pes
     * @description
     * attr BUTTON에 Ctrl+ 마우스 우클릭을 통해 vscode로
     * 해당 source를 호출 하는 기능 추가.
     */
    parent.DEV_SOURCE_FINDER.setRunScriptPath(oRBtn1);
     ...
     * @author  pes
     * @description
     * attr CHECKBOX에 Ctrl+ 마우스 우클릭을 통해 vscode로
     * 해당 source를 호출 하는 기능 추가.
     */
    parent.DEV_SOURCE_FINDER.setRunScriptPath(oRChk1);
     ...
     * @author  pes
     * @description
     * attr의 바인딩/서버이벤트 아이콘에 Ctrl+ 마우스 우클릭을 통해 vscode로
     * 해당 source를 호출 하는 기능 추가.
     */
    parent.DEV_SOURCE_FINDER.setRunScriptPath(oRIcon1);
     ...
     * @author  pes
     * @description
     * attr의 help(script 이벤트) 아이콘에 Ctrl+ 마우스 우클릭을 통해 vscode로
     * 해당 source를 호출 하는 기능 추가.
     */
    parent.DEV_SOURCE_FINDER.setRunScriptPath(oRIcon2);
  ...
  oAPP.fn.attrChange = async function(is_attr, uityp, bSkipRefresh, bForceUpdate){
    ...
    /**
     * @since   2026-03-07 02:38:24
     * @version v3.6.0-2
     * @author  PES
     * @description
     * attribute에 관련하여 예외처리를 수행하는 module js 내용 추가.
     */
  ...
  oAPP.fn.attrResetAttr = function(){
    ...
    parent.showMessage(sap, 30, "I", l_msg, async function(param){
      ...
      /**
       * @since   2026-01-28 20:57:39
       * @version v3.5.8-1
       * @author  pes
       * @description
       * UI의 attribute 변경 항목 필터 버튼 바인딩 초기화 및 ui attribute 리스트 filter 초기화 처리.
       */
      let {initButtonBind, clearDataFilter} = await import(parent.PATH.join(oAPP.oDesign.pathInfo.designRootPath, 
        "attributesArea", "changedDataFilter.js"));
      
      //변경 항목 필터 버튼 바인딩 초기화.
      initButtonBind();

      //ui attribute 리스트 filter 초기화.
      clearDataFilter({oUi:oAPP.attr.ui.oRTab1});
   ...
   ************************************************************************/
  oAPP.fn.attrDocumentProc = async function(is_attr){
    ...
    switch(is_attr.UIATK){
      case "DH001022":
         ...
         * @author  PES
         * @description
         * css 링크 팝업 호출전 callback eventBus 등록 처리
         * css 링크 팝업에서 값을 변경 후 저장시 ROOT의 ATTR 변경 처리를 위한 이벤트 등록.
         */
        async function _cssCallback(){
          ...
          sap.ui.getCore().getEventBus().unsubscribe("WS20POPUP", "cssJsLinkAddPopupCallback", _cssCallback);
        ...
        //CSS LINK 팝업의 event bus callback 처리 이벤트 등록.
        sap.ui.getCore().getEventBus().subscribe("WS20POPUP", "cssJsLinkAddPopupCallback", _cssCallback);
      ...
      case "DH001023":
         ...
         * @author  PES
         * @description
         * JS 링크 팝업 호출전 callback eventBus 등록 처리
         * JS 링크 팝업에서 값을 변경 후 저장시 ROOT의 ATTR 변경 처리를 위한 이벤트 등록.
         */
        async function _jsCallback(){
          ...
          sap.ui.getCore().getEventBus().unsubscribe("WS20POPUP", "cssJsLinkAddPopupCallback", _jsCallback);
        ...
        //JS LINK 팝업의 event bus callback 처리 이벤트 등록.
        sap.ui.getCore().getEventBus().subscribe("WS20POPUP", "cssJsLinkAddPopupCallback", _jsCallback);
      ...
      case "DH001026":
         ...
         * @author  PES
         * @description
         * Web Security Settings 팝업 호출전 callback eventBus 등록 처리
         * Web Security Settings 팝업에서 값을 변경 후 저장시 ROOT의 ATTR 변경 처리를 위한 이벤트 등록.
         */
        async function _wsCallback(){
          ...
          sap.ui.getCore().getEventBus().unsubscribe("WS20POPUP", "webSecurityPopupCallback", _wsCallback);
        ...
        //JS LINK 팝업의 event bus callback 처리 이벤트 등록.
        sap.ui.getCore().getEventBus().subscribe("WS20POPUP", "webSecurityPopupCallback", _wsCallback);
      ...
      case "DH001106":
        ...
        /**
         * @since   2026-05-08 18:15:50
         * @version v3.6.0-4
         * @author  pes
         * @description
         * 기존 callInitPreScreenPopup.js 에서 node module로 구성되던 내용을
         * js module로 변경 처리. 이에 따른 script 호출 방식에서 js module import 방식으로 변경 처리.
         */
        // //Use init pre-screen event 사용 여부 확인 팝업 호출.
        // parent.require(parent.PATH.join(oAPP.oDesign.pathInfo.designRootPath, 
        //   "documents", "callInitPreScreenPopup.js"))(is_attr);

        const {default: initPreScreenPopup } = await import(parent.PATH.join(oAPP.oDesign.pathInfo.designRootPath, 
          "documents", "callInitPreScreenPopup", "index.js"));

        initPreScreenPopup(is_attr);
          
        return true;

      /**
       * @since   2026-02-09 00:25:24
       * @version v3.6.0-0
       * @author  PES
       * @description
       * Dump Write 처리에 따른 팝업 호출 처리 로직 추가.
       */
      case "DH001091":

        //Dump Write HTML PATH 구성.
        const _htmlPath = parent.PATH.join(oAPP.oDesign.pathInfo.designRootPath, "html", "documents", 
          oAPP.oDesign.settings.GLANGU, "dumpWritePopup", "index.html");

        
        //HTML 내용 읽기.
        let _content;
        try {
          _content = parent.FS.readFileSync(_htmlPath);
        } catch (error) {
          
          let _msg = `"www/ws10_20/design/js/uiAttributeArea.js"\n` +
            `파일에서 오류 발생.\n` +
            `Dump Write HTML 정보를 얻을 수 없음\n` +
            `html path ${_htmlPath}\n` +
            `관리자에게 문의 바랍니다.`;
        ...
        /**
         * @since   2026-04-21 11:08:31
         * @version v3.6.0-4
         * @author  pes
         * @description
         * dialogViewer module의 resolve 처리에서 callback function으로 
         * fnCallback이 추가됨에 따라 기존 then() 구문에서 fnCallback으로 변경 처리.
         */
        // //DIALOG VIEWER 호출.
        // dialogViewer({htmlContent:_html}, {
        //   title:parent.WSUTIL.getWsMsgClsTxt("", "ZMSG_WS_COMMON_001", "492"), //492	덤프 이력 기록
        //   width:"50%", 
        //   height:"50%", 
        //   draggable:true,
        //   resizable:true,
        ...
        //   is_attr.UIATV = sRes?.IF_DATA.UIATV || "";


        //   //attribute 입력건에 대한 미리보기, attr 라인 style 등에 대한 처리.
        //   oAPP.fn.attrChangeProc(is_attr, undefined ,false, true);
  ...
  //프로퍼티 f4 help 호출 처리.
  oAPP.fn.attrCallValueHelp = async function(oUi, is_attr){
    ...
    /**
     * @since   2026-04-24 14:41:15
     * @version v3.6.0-4
     * @author  pes
     * @description
     * styleClass의 f4 help 선택시 미리 정의된 css 속성 확인 및 
     * 선택 팝업 호출 처리 로직 추가.
     */
    if(is_attr.UIATT === "styleClass"){

      let {styleClassPopup} = await import(parent.PATH.join(oAPP.oDesign.pathInfo.designRootPath, 
        "attributesArea", "styleClassPopup.js"));

      styleClassPopup({oUi:oUi, sAttr: is_attr});
  ...
  //OBJID 입력건 처리.
  oAPP.fn.attrChnageOBJID = async function(oEvent){
  ...
  oAPP.fn.attrChgAttrVal = function(is_attr, uitp){
    ...
    function lf_add_T_0015(){
      ...
      if(l_indx !== -1){
        ...
        oAPP.attr.prev[is_attr.OBJID]._T_0015[l_indx].SHCUT = is_attr.SHCUT;
        return;
      }
  ...
  oAPP.fn.updateDOCAttrList = async function(OBJID){
    ...
    for(var i=0, l= lt_ua003.length; i<l; i++){
      ...
      switch(lt_ua003[i].ITMCD){
        ...
        /**
         * @since   2026-02-07 12:58:41
         * @version v3.6.0-0
         * @author  pes
         * @description
         * 서버이벤트 수행중 덤프 발생시 st22 덤프정보를 write하는 기능이
         * 추가됨에 따라 document에서 해당 기능을 설정할 수 있도록
         * 버튼 속성 활성화 로직 추가.
         */
    ...
    let _oPromise = _oRender.setAfterRendering(oAPP.attr.ui.oRTab1);
  ...
  oAPP.fn.attrSetLineStyle = function(is_attr){
    ...
    switch(is_attr.UIATY){
      case "1": //프로퍼티
        ...
        /* 3.5.6-16 2025-11-19 16:56:10 end */
    ...
    /**
     * @since   2026-03-07 02:36:52
     * @version v3.6.0-2
     * @author  PES
     * @description
     * attr 영역의 입력 필드에 관련하여 공통코드 UW11를 기준으로 ui 및 속성 정보를 
     * 예외처리 하는 내용 추가.
     */
    //UI Attribute 입력필드 표현 내용 설정건에 해당되는지 확인.
     ...
     * @author  PES
     * @description
     * attr 영역의 바인딩, 클라이언트 이벤트 아이콘에 관련하여 공통코드 UW12를 기준으로 ui 및 속성 정보를 
     * 예외처리 하는 내용 추가.
     */
    //UI Attribute 아이콘 표현 내용 설정건에 해당되는지 확인.
  ...
  oAPP.fn.getServerEventList = async function(f_cb, bSkipUnLock){
    ...
    return new Promise((resolve, reject) => {
      ...
      sendAjax(oAPP.attr.servNm + "/getServerEventList", oFormData, function(param){
         ...
         * @author  PES
         * @description
         * 서버이벤트 검색 요청시 busy 처리를 안하도록 파라메터 추가 처리.
         * function 호출처에서 busy 처리를 안하고 lock처리 이후 서버이벤트를
         * 검색하는 경우가 발생됨에 따라 서버요청시 busy 처리를 안하도록 로직 보완 처리.
         */
  ...
  oAPP.fn.updateAttrList = function(UIOBK, OBJID){
    ...
    return new Promise(async function(resolve, reject){
        ...
        if(OBJID === "ROOT"){
          ...
          await oAPP.fn.updateDOCAttrList(OBJID);
        ...
        /**
         * @since   2026-01-02 10:31:06
         * @version v3.5.7-4
         * @author  PES
         * @description
         * UI를 선택할때마다 서버이벤트 리스트를 검색하는 로직 주석 처리.
         * 서버이벤트 DDLB를 펼칠때 서버이벤트를 검색하도록 로직 보완.
         */
        // //서버 이벤트 항목 검색.
  ...
  oAPP.fn.attrChkBindAggrPossible = function(is_attr, bSkipMsg){
    ...
    //WS Patch 3.5.7-4 패치 이전 버전의 서버 인경우 하위 점검 로직 SKIP.
    //(해당 패치 부터 단축키 관련 로직이 추가됨)
    if(oAPP.common.checkWLOList("C", "UHAK901289") !== true){
      return;
    }


    //입력한 UI로부터 하위를 탐색하며 단축키가 등록된 이벤트 정보 얻기.
    let _sShortcutEvent = oAPP.fn.attrGetShortcutEvent(is_attr.OBJID);
    ...
    if(!bSkipMsg){
      //486	&1 UI의 &2 이벤트에 단축키가 등록되어 바인딩을 진행할 수 없습니다.
      parent.showMessage(sap, 10, "E", parent.WSUTIL.getWsMsgClsTxt("", "ZMSG_WS_COMMON_001", "484", _sShortcutEvent.OBJID, _sShortcutEvent.UIATT));
    }
  ...
  oAPP.fn.attrGetShortcutEvent = function(OBJID){
    ...
    //현재 ui의 tree 정보 얻기.
    let _sDesignUi = oAPP.fn.getTreeData(OBJID);
    ...
    //단축키 
    let _s0015 = oAPP.attr.prev[_sDesignUi.OBJID]._T_0015.find( item => item.UIATY === "2" && Object.keys(item.SHCUT).length > 0 );

    if(_s0015){
      return _s0015;
    }

    if(_sDesignUi.zTREE.length === 0){
      return;
    }

    for(let _sChild of _sDesignUi.zTREE){
      
      //하위를 탐색하며 단축키 등록건 존재 여부 확인.
      let _sShortcutEvent = oAPP.fn.attrGetShortcutEvent(_sChild.OBJID);

      if(_sShortcutEvent){
        return _sShortcutEvent;
      }
  ...
  //ui의 bindingContext에서 라인 데이터 정보 얻기.
  oAPP.oDesign.fn.getBindContextProperty = function(oUi){
