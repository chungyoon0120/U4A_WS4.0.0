 ...
 * ws_events.js
 **************************************************************************
 * 오류코드 접두: WSEV / 다음 번호: 020   (018 사용, 019 는 아직 빈 번호)
 ...
 *  WSEV-018  Save·Activate·Syntax Check — [서버 처리 後] 선택 UI 다시 클릭(_ws20ReselectSelectedUI) 실패
 *
...
(function (window, $, oAPP) {
    ...
    oAPP.events.ev_pressSyntaxCheckBtn = function (oEvent) {
        ...
        sendAjax(sPath, oFormData, (oResult) => {
            ...
            if (oResult.RETCD != "E") {
                ...
                // [2026-09-03 장군님 지시] 서버 왕복까지 끝난 시점 → 왼쪽 UI 트리의 선택 줄 다시 클릭.
                _ws20ReselectSelectedUI("Syntax Check(구문 검사)");
    ...
    /************************************************************************
     * [공통] 서버 왕복이 끝난 뒤 왼쪽 UI 트리의 선택 줄을 "다시 클릭"한다.
     *   장군님 지시 2026-09-03 — Save(저장) / Activate(활성화) / Syntax Check(구문 검사)
     *   가 서버까지 갔다가 다 끝나는 시점에, 지금 선택돼 있는 UI 를 한 번 더 클릭한
     *   효과를 준다(속성 영역·미리보기 선택이 클릭과 똑같이 다시 맞춰진다).
     *
     *   ★새 동작을 만들지 않는다 — 트리 클릭 핸들러를 그대로 다시 부르는
     *     fnWs20TreeReselectCurrent(ws_html5_ws20_tree.js) 에 위임한다.
     *   ★이 시점은 서버 저장이 이미 끝난 뒤라 되돌릴 수 없다. 그래서 여기서 터져도
     *     저장을 되돌리지 않고, 조용히 삼키지도 않고 코드로 드러내기만 한다(WSEV-018).
     *   선택된 줄이 없으면 아무것도 하지 않는다.
     ************************************************************************/
    function _ws20ReselectSelectedUI(sWhere) {

        try {

            if (typeof oAPP.fn.fnWs20TreeReselectCurrent !== "function") {
                throw new Error("oAPP.fn.fnWs20TreeReselectCurrent 미정의(WS20 트리 모듈 미로드)");
            }
    ...
    oAPP.events.ev_pressActivateBtn = async function (oEvent) {
        ...
        // 예외검증(chkExcepionAttr). 원본 1317행은 감싸지 않고 직접 호출한다 — 점검이 실패하면
        //   예외가 그대로 터져 활성화도 진행되지 않는다(fail-closed).
        // [BR55] 기존 HTML5 는 이 호출을 try/catch 로 감싸 점검 실패를 조용히 삼켰고(오류 0건 취급),
        //   그 결과 오류가 있는 앱이 그대로 활성화될 여지가 있었다. 점검 실패는 삼키지 말고
        //   구간 오류코드로 표면화한 뒤 활성화를 중단한다(조용한 catch 금지 규칙).
        ...
        if (iexceplength !== 0) {
            ...
            // 이 안에서 화면이 로드가 완료되면
            // IPC로 비지 끄는 로직 있음
            //[BR55] 오류목록 표시 실패도 조용히 넘기지 않는다(표시가 죽으면 busy 가 풀리지 않아 화면이 멈춘다).
            try {
                if (typeof oAPP.fn.fnMultiFooterMsg !== "function") {
                    throw new Error("oAPP.fn.fnMultiFooterMsg 미정의(오류목록 표시 모듈 미로드)");
                }
        ...
        sendAjax(sPath, oFormData, function (oResult) {
            ...
            // [2026-09-03 장군님 지시] 서버 왕복까지 끝난 시점 → 왼쪽 UI 트리의 선택 줄 다시 클릭.
            _ws20ReselectSelectedUI("Activate(활성화)");
    ...
    oAPP.events.ev_pressSaveBtn = function (oEvent) {
        ...
        function lf_getAppInfo(oResult) {
            ...
            // [2026-09-03 장군님 지시] 서버 왕복까지 끝난 시점 → 왼쪽 UI 트리의 선택 줄 다시 클릭.
            //  ※ 위쪽 ISBACK(WS10 로 이동) / ISDISP(Display 모드 전환) 분기는 화면 자체가 바뀌므로
            //    그 앞에서 이미 return 되어 여기 오지 않는다(다시 클릭도 하지 않는다).
            _ws20ReselectSelectedUI("Save(저장)");
