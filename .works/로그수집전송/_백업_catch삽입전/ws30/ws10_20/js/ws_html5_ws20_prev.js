...
(function (window, $, oAPP) {
    ...
    function _installPrevErrorSurface(oFrame) {
        ...
        function lf_wrapEngine() {
            ...
            var oSapUi = null;
            try { oSapUi = w.sap && w.sap.ui; } catch (e) { oSapUi = null; }
            ...
            if (!oSapUi || typeof oSapUi.getCore !== "function" || typeof oSapUi.requireSync !== "function") {
                ...
                setTimeout(function () { try { lf_wrapEngine(); } catch (e) { } }, 200);
                return;
    ...
    oAPP.fn.fnWs20EnsurePreviewModule = function (fnDone) {
        ...
        function lf_installPreviewMoveDelegate() {
            ...
            // ★ callDesignContextMenu.js(작업폴더 옛 원본) 로드가 chkUiCardinality 를 옛 문구(021)로 되덮는다.
            //   WS20 원본(WS_DESIGN 최신)은 바인딩 다건 안내를 새 문구(ZMSG_WS_COMMON_002/000)로 교체했으므로,
            //   그 파일 로드 직후 내 새 문구 버전(edit.js 의 _ws20ChkUiCardinality)으로 되돌린다(멱등, 붙여넣기 재감싸기와 동일 자리).
            if (typeof oAPP.fn._ws20ChkUiCardinality === "function") {
                oAPP.fn.chkUiCardinality = oAPP.fn._ws20ChkUiCardinality;
            }
        }
    ...
    function _ws20ReleasePrevBusy() {
        ...
        // [BR60] 미리보기 로드 종료(성공·실패 공통) 알림 — 대기 등록된 1회성 콜백을 실행한다.
        //   미리보기 로더는 자기 화면잠금만 관리하므로, "미리보기가 다 뜬 뒤에 풀어야 하는"
        //   다른 잠금(레이아웃 저장의 단축키 잠금 등)이 이 시점을 받아 갈 수 있게 한다.
        //   등록: oAPP.attr.__ws20PrevDoneCbs.push(fn). 실행 후 목록은 비운다(중복 실행 방지).
        try {
