...
| # | 항목 | 원본 | HTML5 | 판단 |
|---|---|---|---|---|
| ΔC1 | **임베디드 속성 배지(EMATT)** | 이름칸 우측에 `ObjectStatus{text:EMATT, icon:EMATT_ICON}` 렌더(designTree.js:4093~4097). 색채움=0:1, 차원=0:N | ✅ **수정: rowHook 에서 이름칸에 `.u4aBwpEmatt` 배지 덧댐**(designArea.js `_ematBadge`+rowHook, frame.css). 아이콘/색/크기 = **WS20 UI 트리(js/ws_html5_ws20_tree.js:193)와 완전 동일** 0:1 fa-regular fa-square / 0:N fa-regular fa-clone, --accent | ✅ **처리완료(2026-07-24)** — C-4 확인대기. 공통 미수정(스코프). WS20 통일. |
